...
void test_dense_matrix_mult()
{
  ...
  }
  {
    ...
    array_ref<double, 2> A(a.data(), {3, 3});
    REQUIRE(A.num_elements() == a.size());
    array<double, 1> WORK(iextensions<1u>{std::max(ma::geqrf_optimal_workspace_size(A), 
...
void test_dense_mat_mul_device(Allocator& alloc)
{
  ...
  //SECTION("mat_herm_ref")
  {
    ...
    array_ref<T, 2, typename std::allocator_traits<Allocator>::pointer> Mref(M.origin(), M.extensions());
...
void test_dense_gerf_gqr_device(Allocator& alloc)
{
  ...
  using T = typename Allocator::value_type;
  {
    ...
    size_t sz = std::max(ma::geqrf_optimal_workspace_size(A), ma::gqr_optimal_workspace_size(A));
...
void test_dense_gerf_gqr_strided_device(Allocator& alloc)
{
  using T = typename Allocator::value_type;
  {
    ...
    size_t sz = std::max(ma::geqrf_optimal_workspace_size(A[0]), ma::gqr_optimal_workspace_size(A[0]));
    array<T, 2, Allocator> WORK({2, sz}, alloc);
    array<T, 2, Allocator> Id({3, 3}, alloc);
    using IAllocator = typename std::allocator_traits<Allocator>::template rebind_alloc<int>;
    array<int, 1, IAllocator> info(iextensions<1u>{2}, IAllocator{});
...
void test_dense_geqrf_getri_batched_device(Allocator& alloc)
{
  ...
  using IAllocator = typename std::allocator_traits<Allocator>::template rebind_alloc<int>;
  array<int, 1, IAllocator> info(iextensions<1u>{2}, IAllocator{});
  array<int, 1, IAllocator> piv(iextensions<1u>{2 * (3 + 1)}, IAllocator{});
  array<T, 2, Allocator> B({3, 3}, 0.0, alloc), Bi({3, 3}, 0.0, alloc);
  array<int, 1, IAllocator> spiv(iextensions<1u>{4}, IAllocator{});
  int status;
  //SECTION("getrf_batched")
  {
    ma::getrfBatched(3, A_array.data(), 4, ma::pointer_dispatch(piv.origin()), ma::pointer_dispatch(info.origin()), 2);
    copy_n(a2.data(), a2.size(), B.origin());
    ma::getrf(B, spiv, WORK);
  }
  //SECTION("getri_batched")
  {
    ma::getriBatched(3, A_array.data(), 4, ma::pointer_dispatch(piv.origin()), Ai_array.data(), 3,
