...
void ma_blas_tests()
{
  ...
  {
    array_ref<double, 1> V(v.data(), iextensions<1u>{long(v.size())});
    ma::scal(2., V);
    {
      vector<double> v2 = {2., 4., 6.};
      array_ref<double, 1> V2(v2.data(), iextensions<1l>{v2.size()});
      verify_approx(V, V2);
    }
  }
  ...
  {
    vector<double> m_ = {9., 24., 30., 9., 4., 10., 12., 7., 14., 16., 36., 1.};
    array_ref<double, 2> M_(m_.data(), {3, 4});
    REQUIRE(M_[2][0] == 14.);
  ...
  }
  {
    ...
    ma::gemv<'T'>(1., M_({0, 2}, {0, 2}), X, 0., Y); // y := M x
  ...
  {
    vector<double> m_ = {9., 24., 30., 4., 10., 12., 14., 16., 36., 4., 9., 1.};
    array_ref<double, 2> M_(m_.data(), {4, 3});
    REQUIRE(M_[2][0] == 14.);
