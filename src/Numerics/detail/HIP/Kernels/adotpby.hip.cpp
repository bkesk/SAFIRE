...
__global__ void kernel_adotpby(int N, T const alpha, T const* x, int const incx,
   ...
   // RUNTIME_CHECK(blockIdx.x==0 and blockIdx.y==0 and blockIdx.z==0)
...
{
  hipLaunchKernelGGL(kernel_adotpby, dim3(1), dim3(1024), 0, 0, N, static_cast<thrust::complex<double> const>(alpha),
...
{
  hipLaunchKernelGGL(kernel_adotpby, dim3(1), dim3(1024), 0, 0, N, static_cast<thrust::complex<float> const>(alpha),
