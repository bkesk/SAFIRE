...
__global__ void kernel_dot_wabn(int nwalk,
                                ...
                                int incy)
{
  if (blockIdx.x >= nwalk || blockIdx.y >= nocc || blockIdx.z >= nocc)
    return;
  ...
  thrust::complex<T2> const* A_(Tab + ((blockIdx.x * nocc + blockIdx.y) * nocc + blockIdx.z) * nchol);
  thrust::complex<T2> const* B_(Tab + ((blockIdx.x * nocc + blockIdx.z) * nocc + blockIdx.y) * nchol);
                                ...
                                int incy)
{
  ...
  thrust::complex<T> alp = static_cast<thrust::complex<T>>(alpha);
  thrust::complex<T2> const* A_(Tab + blockIdx.x * nocc * nact * nchol);
  for (int n = 0; n < nt; n++, nb += blockDim.y)
  {
    ...
    int i = nb / nocc;
    int b = nb % nocc;
    cache[id] += static_cast<thrust::complex<T>>(A_[(a * nchol + i) * nact + b] * A_[(b * nchol + i) * nact + a]);
  }
              ...
              int incy)
{
  ...
  kernel_dot_wanb<<<grid_dim, block_dim>>>(nf, nwalk, nocc, nact, nchol, static_cast<thrust::complex<double> const>(alpha),
                                           reinterpret_cast<thrust::complex<double> const*>(Tab),
                                           reinterpret_cast<thrust::complex<double>*>(y), incy);
              ...
              int incy)
{
  ...
  dim3 grid_dim(nwalk, na, nb);
  dim3 block_dim(a_, b_, 1);
  kernel_dot_wanb<<<grid_dim, block_dim>>>(nf, nwalk, nocc, nact, nchol, static_cast<thrust::complex<float> const>(alpha),
                                           reinterpret_cast<thrust::complex<float> const*>(Tab),
                                           reinterpret_cast<thrust::complex<float>*>(y), incy);
  qmc_cuda::cuda_check(cudaGetLastError(), "dot_wanb");
  qmc_cuda::cuda_check(cudaDeviceSynchronize(), "dot_wanb");
