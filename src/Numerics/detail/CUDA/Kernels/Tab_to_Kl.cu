                                  ...
                                  thrust::complex<T>* Kl, int ldkl)
{
  ...
  if (w < nwalk)
  {
    for (int a = 0; a < nocc; a++)
    {
      thrust::complex<T> const* Tab_(Tab + ((w * nocc + a) * nchol) * nact + a);
		...
		int ldkl)
{
  dim3 grid_dim(nwalk, 1, 1);
  int nthr = std::min(256, nchol);
  kernel_Tanb_to_Kl<<<grid_dim, nthr>>>(nwalk, nocc, nchol, nact,
                                        reinterpret_cast<thrust::complex<double> const*>(Tab),
                                        reinterpret_cast<thrust::complex<double>*>(Kl),ldkl);
  qmc_cuda::cuda_check(cudaGetLastError(), "Tanb_to_Kl");
  qmc_cuda::cuda_check(cudaDeviceSynchronize(), "Tanb_to_Kl");
}
