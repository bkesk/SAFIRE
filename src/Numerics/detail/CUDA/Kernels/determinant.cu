                                                      ...
                                                      int nbatch)
{
  ...
  if (threadIdx.x == 0)
  {
    ...
    *(det + batch*incx) = sg[0] * exp(tmp[0] - LogOverlapFactor);
  }
  __syncthreads();
}
                                                      ...
                                                      int nbatch)
{
  ...
  if (threadIdx.x == 0)
  {
    ...
    for (int i = 1; i < imax; i++)
      tmp[0] += tmp[i];
    *(det + batch*incx) = exp(tmp[0] - LogOverlapFactor);
  }
  __syncthreads();
}
                                              ...
                                              thrust::complex<T>* logdet)
{
  ...
  if (threadIdx.x == 0)
  {
    ...
    for (int i = 1; i < imax; i++)
      tmp[0] += tmp[i];
    *logdet = thrust::complex<T>(tmp[0] - LogOverlapFactor, 0.0);
  }
  __syncthreads();
}
...
void strided_determinant_from_getrf_gpu(int N,
                                        ...
                                        int nbatch)
{
  kernel_strided_determinant_from_getrf<<<nbatch, 64>>>(N, m, lda, mstride, piv, pstride, 
                                                        LogOverlapFactor, res, incx,
                                                        nbatch);
  qmc_cuda::cuda_check(cudaGetLastError());
  qmc_cuda::cuda_check(cudaDeviceSynchronize());
}
                                        ...
                                        int nbatch)
{
  kernel_strided_determinant_from_getrf<<<nbatch, 64>>>(N, reinterpret_cast<thrust::complex<double>*>(m), lda, mstride,
                                                        piv, pstride,
                                                        static_cast<thrust::complex<double>>(LogOverlapFactor),
                                                        reinterpret_cast<thrust::complex<double>*>(res), incx, nbatch);
  qmc_cuda::cuda_check(cudaGetLastError());
  qmc_cuda::cuda_check(cudaDeviceSynchronize());
}
...
void batched_determinant_from_getrf_gpu(int N,
                                        ...
                                        int nbatch)
{
  kernel_batched_determinant_from_getrf<<<nbatch, 64>>>(N, m, lda, piv, pstride, LogOverlapFactor, res, incx, nbatch);
  qmc_cuda::cuda_check(cudaGetLastError());
  qmc_cuda::cuda_check(cudaDeviceSynchronize());
}
