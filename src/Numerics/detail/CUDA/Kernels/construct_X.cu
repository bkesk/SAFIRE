...
#include "Numerics/detail/CUDA/Kernels/probit.hpp"
                                   ...
                                   thrust::complex<T3>* X)
{
  ...
  {
    if (abs(vmf_[m]) > vbound * sqrtdt)
      vmf_t = vmf_[m] / (abs(vmf_[m]) / (vbound * sqrtdt));
    else
      vmf_t = vmf_[m];
    ...
    cache[2 * threadIdx.x + 1] += im * static_cast<thrust::complex<T>>(X_[m * dX]) * (vmf_[m]);
  }
                                                   ...
                                                   thrust::complex<T1>* X)
{
  ...
  for (int m = threadIdx.x; m < nCV; m += blockDim.x)
    cache[threadIdx.x] += im * static_cast<thrust::complex<T>>(X_[m * dX]) * vmf_[m];
...
// MF[ni,iw] = sum_m ( im * X[m,ni,iw] * vMF[m] )
template<typename T, typename T2, typename T3>
__global__ void kernel_construct_X_model(int nCV,
                                   int nsteps,
                                   int nwalk,
                                   T sqrtdt,
                                   T vbound,
                                   ...
                                   thrust::complex<T3>* X)
{
  ...
  thrust::complex<T3>* X_(X + ni * nwalk + iw);
  // assume RNs[ni][iw][m]  
//  T const* rng_(RNs + ( ni * nwalk + iw ) * nCV );
// mimiking cpu usage rng[m][nstep][w] 
T const* rng_(RNs + ni * nwalk + iw);
  int dvb_ = nwalk;
  int dX   = nsteps * nwalk;
  ...
  for (int m = threadIdx.x; m < nCV; m += blockDim.x)
  {
       ...
       FieldTypes[m] == 1 ) 
    {
      ...
      thrust::complex<T> vdiff = (im * (vb_t - vmf_t));
      //double r = kernels::probit(rng_[m]);  
double r = kernels::probit(rng_[m * dX]);  
      X_[m * dX] = static_cast<thrust::complex<T3>>(r) +
                   static_cast<thrust::complex<T3>>(vdiff);
      cache[2 * threadIdx.x] -= vdiff * (static_cast<thrust::complex<T>>(X_[m * dX]) - 0.5 * vdiff);
      cache[2 * threadIdx.x + 1] += im * static_cast<thrust::complex<T>>(X_[m * dX]) * (vmf_[m]);
//    } else if {
// how do I return error codes?
    } else {
      X_[m * dX] = static_cast<thrust::complex<T3>>(123456789.0);;
      cache[2 * threadIdx.x] += thrust::complex<T>(-123456789);
      cache[2 * threadIdx.x + 1] += thrust::complex<T>(-123456789);
    }
  }
...
template<typename T, typename T1>
__global__ void kernel_construct_X_model_free_projection(int nCV,
                                                   int nsteps,
                                                   int nwalk,
                                                   T sqrtdt,
                                                   int const* FieldTypes,
                                                   ...
                                                   thrust::complex<T1>* X)
{
  ...
  for (int m = threadIdx.x; m < nCV; m += blockDim.x) {
    ...
    { 
      double r = kernels::probit(rng_[m]);
      X_[m * dX] = static_cast<thrust::complex<T1>>(r); 
      cache[threadIdx.x] += im * static_cast<thrust::complex<T>>(X_[m * dX]) * vmf_[m];
    } else {
      X_[m * dX] = static_cast<thrust::complex<T1>>(123456789.0);;
      cache[threadIdx.x] += thrust::complex<T>(-123456789);
    }
  }
                 ...
                 std::complex<double>* X)
{
  ...
  dim3 grid_dim(nsteps, nwalk, 1);
  if (free_projection)
    kernel_construct_X_model_free_projection<<<grid_dim, block_dim>>>(nCV, nsteps, nwalk, sqrtdt,FieldTypes,
                                                                reinterpret_cast<thrust::complex<double> const*>(vMF),
                                                                reinterpret_cast<thrust::complex<double>*>(MF),
                                                                RNs,
                                                                reinterpret_cast<thrust::complex<double>*>(X));
  else
    kernel_construct_X_model<<<grid_dim, block_dim>>>(nCV, nsteps, nwalk, sqrtdt, vbound,FieldTypes,
                                                reinterpret_cast<thrust::complex<double> const*>(vMF),
                                                reinterpret_cast<thrust::complex<double> const*>(vbias),
                                                reinterpret_cast<thrust::complex<double>*>(HW),
                                                reinterpret_cast<thrust::complex<double>*>(MF),
                                                RNs,
                                                reinterpret_cast<thrust::complex<double>*>(X));
  qmc_cuda::cuda_check(cudaGetLastError(), "construct_X_model");
  qmc_cuda::cuda_check(cudaDeviceSynchronize(), "construct_X_model");
}
void construct_X_model(int nCV,
                 int nsteps,
                 int nwalk,
