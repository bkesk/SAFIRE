...
#ifndef SPARSE_CUDA_GPU_PTR_HPP
#define SPARSE_CUDA_GPU_PTR_HPP
...
template<typename T, typename Q1, typename Q2, typename I1, typename I2>
void csrmv(const char transa,
           const int M,
           const int K,
           const T alpha,
           const char* matdescra,
           device::device_pointer<Q1> const A,
           device::device_pointer<I1> const indx,
           device::device_pointer<I2> const pntrb,
           device::device_pointer<I2> const pntre,
           device::device_pointer<Q2> const x,
           const T beta,
           device::device_pointer<T> y, device_cuda_backend)
{
  ...
  if (bufferSize > 0)
  {
    ...
    using pointer_t = typename std::allocator_traits<typename DeviceBufferManager::template allocator_t<T>>::pointer;
           ...
           const int nbatch)
{
  ...
  {
    using sfqmc::afqmc::DeviceBufferManager;
    using pointer_t = typename std::allocator_traits<typename DeviceBufferManager::template allocator_t<char>>::pointer;
    DeviceBufferManager buffer_manager;
