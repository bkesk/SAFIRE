...
#include "Numerics/detail/sparse.hpp" // IWYU pragma: keep
...
{
  RUNTIME_CHECK(arg(A).size(0) == arg(A).size(1), "");
  typename std::decay<MultiArray2D>::type::element one(1.0);
...
template<
    ...
    typename = typename std::enable_if<MultiArray2DA::dimensionality == 2 and MultiArray2DB::dimensionality == 2 and
                                       std::decay<MultiArray2DC>::type::dimensionality == 2>::type>
MultiArray2DC&& add(MultiArray2DA const& A, MultiArray2DB const& B, MultiArray2DC&& C)
{
  using elementC = typename std::decay_t<MultiArray2DC>::element;
  return ma::geam<op_tag<MultiArray2DA>::value, op_tag<MultiArray2DB>::value>(elementC(1.0), arg(A), 
				elementC(1.0), arg(B), std::forward<MultiArray2DC>(C));
}
...
template<
    ...
    class MultiArrayB,
    class MultiArray3DC,
    typename = typename std::enable_if_t<MultiArrayA::dimensionality == 2 or 
                                         MultiArrayA::dimensionality == 3>,
    typename = typename std::enable_if_t<MultiArrayB::dimensionality == 2 or 
                                         MultiArrayB::dimensionality == 3>, 
    typename = typename std::enable_if_t<std::decay_t<MultiArray3DC>::dimensionality == 3>
    >
MultiArray3DC&& productStridedBatched(T alpha,
                                      ...
                                      MultiArray3DC&& C)
{
  return ma::gemmStridedBatched<op_tag<MultiArrayB>::value, op_tag<MultiArrayA>::value>(
                alpha, arg(B), arg(A), beta, std::forward<MultiArray3DC>(C));
}

// specialization for A * Bi = Ci, A a sparse matrix
template<
    class T,
    class SparseArrayA,
    class MultiArray3DB,
    class MultiArray3DC,
    typename = typename std::enable_if_t<SparseArrayA::dimensionality == -2>,
    typename = typename std::enable_if_t<MultiArray3DB::dimensionality == 3>,
    typename = typename std::enable_if_t<std::decay_t<MultiArray3DC>::dimensionality == 3>,
    typename = void
    >
MultiArray3DC&& productStridedBatched([[maybe_unused]] T alpha,
                                      [[maybe_unused]] MultiArray3DB const& B,
                                      [[maybe_unused]] SparseArrayA const& spA,
                                      [[maybe_unused]] T beta,
                                      MultiArray3DC&& C)
{
  APP_ABORT(" Error: Strided Batched DenseMatrix x SparseMatrix product not implemented. \n\n");
...
template<
    ...
    >
MultiArray3DC&& productStridedBatched(T alpha,
                                      SparseArrayA const& spA,
                                      MultiArray3DB const& B,
                                      T beta,
                                      MultiArray3DC&& C)
{
  int nbatch = C.size();
  RUNTIME_CHECK(arg(B).size() >= nbatch, "");

  using elementA = typename SparseArrayA::element;
  //using elementB = typename MultiArray3DB::element;
  //using elementC = typename std::decay_t<MultiArray3DC>::element;
  RUNTIME_CHECK(op_tag<SparseArrayA>::value == 'N' || op_tag<SparseArrayA>::value == 'T' ||
         op_tag<SparseArrayA>::value == 'C' || op_tag<SparseArrayA>::value == 'H', "");
  RUNTIME_CHECK(op_tag<MultiArray3DB>::value == 'N', "");
  RUNTIME_CHECK(arg(B).size(2) == C.size(2), "");
  if (op_tag<SparseArrayA>::value == 'N')
  {
    RUNTIME_CHECK(arg(spA).size(0) == std::forward<MultiArray3DC>(C).size(1), "");
    RUNTIME_CHECK(arg(spA).size(1) == arg(B).size(1), "");
  }
  else
  {
    RUNTIME_CHECK(arg(spA).size(0) == arg(B).size(1), "");
    RUNTIME_CHECK(arg(spA).size(1) == std::forward<MultiArray3DC>(C).size(1), "");
  }

  ma::csrmm(op_tag<SparseArrayA>::value, arg(spA).size(0), C.size(2), arg(spA).size(1), elementA(alpha), "GxxCxx",
          pointer_dispatch(arg(spA).non_zero_values_data()), pointer_dispatch(arg(spA).non_zero_indices2_data()),
          pointer_dispatch(arg(spA).pointers_begin()), pointer_dispatch(arg(spA).pointers_end()),
          pointer_dispatch(arg(B).origin()), arg(B).stride(1), arg(B).stride(0),
          elementA(beta), pointer_dispatch(C.origin()), C.stride(1), C.stride(0), nbatch,
	  select_backend<MultiArray3DB>());
...
template< class MultiArrayA,
          class MultiArrayB,
          class MultiArray3DC>
MultiArray3DC&& productStridedBatched(MultiArrayA const& A,
                                      MultiArrayB const& B,
                                      MultiArray3DC&& C)
{
    using T = typename std::decay_t<MultiArray3DC>::element;
    return productStridedBatched( T(1.), A, B, T(0.), std::forward<MultiArray3DC>(C) );
}
...
template<
    ...
    class MultiArray1DB,
    class MultiArray1DC,
    typename = typename std::enable_if<SparseMatrixA::dimensionality == -2 and 
...
MultiArray1DC&& product(T alpha, SparseMatrixA const& A, MultiArray1DB const& B, T beta, MultiArray1DC&& C)
{
  ...
  RUNTIME_CHECK(op_tag<SparseMatrixA>::value == 'N' || op_tag<SparseMatrixA>::value == 'T', "");
  RUNTIME_CHECK(op_tag<MultiArray1DB>::value == 'N', "");
  if (op_tag<SparseMatrixA>::value == 'N')
  {
    RUNTIME_CHECK(arg(A).size(0) == std::forward<MultiArray1DC>(C).size(0), "");
    RUNTIME_CHECK(arg(A).size(1) == arg(B).size(0), "");
  }
  else
  {
    RUNTIME_CHECK(arg(A).size(0) == arg(B).size(0), "");
    RUNTIME_CHECK(arg(A).size(1) == std::forward<MultiArray1DC>(C).size(0), "");
  }

  ma::csrmv(op_tag<SparseMatrixA>::value, arg(A).size(0), arg(A).size(1), elementA(alpha), "GxxCxx",
...
MultiArray2DC&& product(T alpha, SparseMatrixA const& A, MultiArray2DB const& B, T beta, MultiArray2DC&& C)
{
  ...
  RUNTIME_CHECK(op_tag<MultiArray2DB>::value == 'N', "");
  RUNTIME_CHECK(arg(B).stride(1) == 1, "");
  RUNTIME_CHECK(std::forward<MultiArray2DC>(C).stride(1) == 1, "");
  if (op_tag<SparseMatrixA>::value == 'N')
  {
    RUNTIME_CHECK(arg(A).size(0) == std::forward<MultiArray2DC>(C).size(0), "");
    RUNTIME_CHECK(arg(A).size(1) == arg(B).size(0), "");
    RUNTIME_CHECK(arg(B).size(1) == std::forward<MultiArray2DC>(C).size(1), "");
  }
  else
  {
    RUNTIME_CHECK(arg(A).size(0) == arg(B).size(0), "");
    RUNTIME_CHECK(arg(A).size(1) == std::forward<MultiArray2DC>(C).size(0), "");
    RUNTIME_CHECK(arg(B).size(1) == std::forward<MultiArray2DC>(C).size(1), "");
  }

  ma::csrmm(op_tag<SparseMatrixA>::value, arg(A).size(0), arg(B).size(1), arg(A).size(1), elementA(alpha), "GxxCxx",
        pointer_dispatch(arg(A).non_zero_values_data()), pointer_dispatch(arg(A).non_zero_indices2_data()),
        pointer_dispatch(arg(A).pointers_begin()), pointer_dispatch(arg(A).pointers_end()),
        pointer_dispatch(arg(B).origin()), arg(B).stride(0), 0, 
        elementA(beta), pointer_dispatch(C.origin()), C.stride(0), 0, 1, select_backend<MultiArray2DB>());
...
template<
    ...
    typename = void // TODO change to use dispatch
    >
MultiArray2DC&& product(
   [[maybe_unused]] T alpha, 
   [[maybe_unused]] MultiArray2DB const& B, 
   [[maybe_unused]] SparseMatrixA const& A,
   [[maybe_unused]] T beta, 
   MultiArray2DC&& C)
{
...
{
  int nbatch = C.size();
  RUNTIME_CHECK(A.size() >= nbatch, "");
  RUNTIME_CHECK(B.size() >= nbatch, "");
  ...
  {
    RUNTIME_CHECK(lda == (*A[i]).stride(0), "");
    RUNTIME_CHECK(ldb == (*B[i]).stride(0), "");
    RUNTIME_CHECK(ldc == (*C[i]).stride(0), "");
    RUNTIME_CHECK(M == (*C[i]).size(1), "");
    RUNTIME_CHECK(N == (*C[i]).size(0), "");
    if (TB == 'N')
    {
      RUNTIME_CHECK(K == (*B[i]).size(0), "");
      RUNTIME_CHECK(M == (*B[i]).size(1), "");
    }
    else
    {
      RUNTIME_CHECK(K == (*B[i]).size(1), "");
      RUNTIME_CHECK(M == (*B[i]).size(0), "");
    }
    if (TA == 'N')
    {
      RUNTIME_CHECK(K == (*A[i]).size(1), "");
      RUNTIME_CHECK(N == (*A[i]).size(0), "");
    }
    else
    {
      RUNTIME_CHECK(K == (*A[i]).size(0), "");
      RUNTIME_CHECK(N == (*A[i]).size(1), "");
    }
                    ...
                    std::vector<MultiArrayPtr2DC>& C)
{
  ...
  RUNTIME_CHECK(TA == 'N' || TA == 'T' || TA == 'C' || TA == 'H', "");
  RUNTIME_CHECK(TB == 'N', "");

  // batched csrmm only with strided interface, :-(
  // if this becomes critical, you can push this loop down one level and use streams on device
  for (int i = 0; i < nbatch; i++)
  {
    ma::csrmm(TA, (*A[i]).size(0), (*B[i]).size(1), (*A[i]).size(1), elementA(alpha), "GxxCxx",
          ...
          pointer_dispatch((*B[i]).origin()), (*B[i]).stride(0), 0, 
          elementA(beta), pointer_dispatch((*C[i]).origin()), (*C[i]).stride(0), 0, 1, 
	  select_backend<pointedType<MultiArrayPtr2DB>>());
  }
}
...
T invert(MultiArray2D&& m, T LogOverlapFactor)
{
  ...
  using iallocator_type = typename std::allocator_traits<allocator_type>::template rebind_alloc<int>;
  ...
  boost::multi::array<int, 1, iallocator_type> pivot(extensions{m.size(0) + 1}, iallocator_type{}); //, iallocator_type{m.get_allocator()});
...
{
  RUNTIME_CHECK(m.size(0) == m.size(1), "");
  RUNTIME_CHECK(pivot.size() >= m.size(0) + 1, "");
  using element = typename std::decay<MultiArray2D>::type::element;
  using sfqmc::afqmc::fill2D;
...
void invert_withSVD(MultiArray2D&& m, MultiArray1DS&& S, MultiArray2DU&& U, MultiArray2DVT&& VT, Buffer&& WORK, T* detvalue){
        RUNTIME_CHECK(m.size(0) == m.size(1), "");
        RUNTIME_CHECK(S.size(0) == m.size(0), "");
        RUNTIME_CHECK(VT.size(0) == m.size(0), "");
        RUNTIME_CHECK(VT.size(0) == VT.size(1), "");
        using element = typename std::decay<MultiArray2D>::type::element;
...
{
  RUNTIME_CHECK(m.size(0) == m.size(1), "");
  RUNTIME_CHECK(pivot.size() >= m.size(0), "");
...
MultiArray2D exp(MultiArray2D const& A, bool printeV = false)
{
  ...
  if (is_hermitian(A))
  {
    ...
    if (printeV)
    {
      ...
      app_log(1, " i    eigV[i]    exp(eigV[i]) ");
      for (int j = 0; j < N; j++)
        app_log(1, " {}  {}  {} ", j, V.first[j], std::exp(V.first[j]));
      app_log(1,
          "************************************************************************************ ");
      app_log(1, "\n\n");
    }
...
{
  if (a.size(0) != b.size(0) or a.size(0) != b.size(0))
    return false;
  using std::abs;
  for (int i = 0; i != a.size(0); ++i)
...
{
  {
    ...
    RUNTIME_CHECK(M.num_elements() == m.size(), "");
...

  {
    ...
    boost::multi::array_ref<double, 2> M(m.data(), {3, 3});
    RUNTIME_CHECK(ma::is_hermitian(M), "");
  }
  {
    std::vector<double> m = {
