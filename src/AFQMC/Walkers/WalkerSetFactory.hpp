...
public:
  ...
  WalkerSet& getWalkerSet(std::shared_ptr<utils::mpi_context_t<boost::mpi3::communicator>> mpi, const std::string& ID, std::shared_ptr<utils::RandomGenerator_t<HOST_MEMORY>> rng)
  {
    auto xml = wlkBlocks.find(ID);
    if (xml == wlkBlocks.end())
...
protected:
  ...
  WalkerSet buildHandler(std::shared_ptr<utils::mpi_context_t<boost::mpi3::communicator>> mpi, ptree pt, std::shared_ptr<utils::RandomGenerator_t<HOST_MEMORY>> rng)
  {
    std::string type, info;
    info = pt.get<std::string>("system", "");
    utils::check(InfoMap.find(info) != InfoMap.end(), "ERROR: Undefined system: {}", info);

    return WalkerSet(WalkerSetBase<MEM>(mpi, pt, InfoMap[info], rng));
  }
