                                                                 ...
                                                                 hdf_archive& hdf_restart)
{
  ...
  nkpts = QN = Idata[2];
  app_log(1," nkpts: ") << nkpts << std::endl;
...
//#define SYMMETRIZE_LQ
  ...
  if (TG.Node().root())
  {
    ...
    TG.Cores().broadcast_n(raw_pointer_cast(QKtoG.origin()), QKtoG.num_elements(), 0);
  }
  TG.Node().barrier();
  ...
  if (TG.Node().root())
  {
    for (int Q = 0; Q < nkpts; Q++)
    {
      ...
      if (kminus[Q] < Q)
      {
        ...
        auto LQ(raw_pointer_cast(LQGun[Q].origin()));
        auto LQm(raw_pointer_cast(LQGun[Qm].origin()));
...
// */
      ...
      {
        RealType scl = 1.0/double(nkpts);
        auto MQ(raw_pointer_cast(rotMuv[Q].origin()));
...
// */
  ...
  nel.first = std::accumulate(raw_pointer_cast(nocc_per_kp[0].origin()), raw_pointer_cast(nocc_per_kp[0].origin()) + nkpts, 0);
  if (type == COLLINEAR)
    nel.second = std::accumulate(raw_pointer_cast(nocc_per_kp[0].origin()) + nkpts,
                                 raw_pointer_cast(nocc_per_kp[0].origin()) + 2 * nkpts, 0);
    ...
    for (int Q = 0; Q < nkpts; Q++)
    {
      if ((nt++) % TG.Global().size() == TG.Global().rank())
      {
        ...
        if (type == COLLINEAR)
        {
          { // Alpha
            ...
            utils::check(Psi.size(0) == nocc_per_kp[nd][Q], "");
            boost::multi::array_ref<ComplexType, 2> haj_r(raw_pointer_cast(haj[nd * nkpts + Q].origin()),
          ...
          { // Beta
            ...
            utils::check(Psi.size(0) == nocc_per_kp[nd][nkpts + Q], "");
            boost::multi::array_ref<ComplexType, 2> haj_r(raw_pointer_cast(haj[nd * nkpts + Q].origin()) +
                                                              nocc_per_kp[nd][Q] * nmo_per_kp[Q],
      ...
      if ((nt++) % TG.Global().size() == TG.Global().rank())
      {
        if (type == COLLINEAR)
        {
          APP_ABORT(" Finish .");
          auto Psi = get_PsiK<boost::multi::array<ComplexType, 2>>(nmo_per_kp, PsiT[2 * nd], Q);
          int ne0  = std::accumulate(raw_pointer_cast(nocc_per_kp[nd].origin()), raw_pointer_cast(nocc_per_kp[nd].origin()) + Q, 0);
          ...
          ne0 = std::accumulate(raw_pointer_cast(nocc_per_kp[nd].origin()) + nkpts,
                                raw_pointer_cast(nocc_per_kp[nd].origin()) + nkpts + Q, 0);
          utils::check(Psi.size(0) == nocc_per_kp[nd][nkpts + Q], "");
          ma::product(ma::H(Piu({ni0, ni0 + nmo_per_kp[Q]}, {0, nmu})), ma::T(Psi),
  ...
  if (TG.Node().root())
  {
    ...
    for (int n = 0; n < cPua.size(); n++)
      TG.Cores().all_reduce_in_place_n(raw_pointer_cast(cPua[n].origin()), cPua[n].num_elements(), std::plus<>());
    for (int n = 0; n < rotcPua.size(); n++)
      TG.Cores().all_reduce_in_place_n(raw_pointer_cast(rotcPua[n].origin()), rotcPua[n].num_elements(), std::plus<>());
  ...
  for(int K=0; K<nkpts; K++) {
    if(K%TG.Node().size() == TG.Node().rank()) {
      boost::multi::array_ref<SPComplexType,2> Likn(raw_pointer_cast(LQKikn[0][K].origin()),
...
*/
  ...
  app_log(1," E1+E0: " << std::setprecision(12) << E0 + 2.0 * E_ << "  ");
  E_ = 0.0;
  for (int K = 0; K < nkpts; K++)
  {
    auto&& G_ = Gc[K];
      ...
      for(int i=0, I=ni0; i<nmo_per_kp[0]; i++, ++I) {
        ...
        for(int j=0, J=nj0; j<nmo_per_kp[0]; j++, ++J) {
          ...
          for(int u=0; u<nmu; u++) {
            ...
            auto lq(raw_pointer_cast(LQGun[Q][nmu*G1+u].origin()));
  ...
  for(int KL=0; KL<nkpts; KL++) {
   for(int KK=0; KK<nkpts; KK++)
   {
    ...
    if((n_++)%TGwfn.Global().size() == TGwfn.Global().rank()) {
      ...
      //boost::multi::array_ref<ComplexType,2> LKI(raw_pointer_cast(LIJ[KI][KK].origin()),{nmo_max*nmo_max,nchol_per_kp[Q]});
      //boost::multi::array_ref<ComplexType,2> LKL(raw_pointer_cast(LIJ[KL][KJ].origin()),{nmo_max*nmo_max,nchol_per_kp[Q]});
...
// */
    ...
    if (Q % TGwfn.Global().size() == TGwfn.Global().rank())
    {
      if (kminus[Q] == Q)
      {
        ...
        for (int KI = 0; KI < nkpts; KI++)
        {
          ...
          boost::multi::array_ref<SPComplexType, 3> LQI(raw_pointer_cast(LQKikn[Q][KI].origin()), {ni, nj, nchol_per_kp[Q]});
          boost::multi::array_ref<SPComplexType, 3> LQJ(raw_pointer_cast(LQKikn[Q][KJ].origin()), {nj, ni, nchol_per_kp[Q]});
          if (KJ > KI)
          {
            for (int i = 0; i < ni; i++)
              for (int j = 0; j < nj; j++)
      ...
      else if (kminus[Q] < Q)
      {
        ...
        for (int KI = 0; KI < nkpts; KI++)
        {
          ...
          int ni = nmo_per_kp[KI];
          int nj = nmo_per_kp[KJ];
          boost::multi::array_ref<SPComplexType, 3> LQm(raw_pointer_cast(LQKikn[Qm][KI].origin()), {ni, nj, nchol_per_kp[Q]});
          boost::multi::array_ref<SPComplexType, 3> LQ(raw_pointer_cast(LQKikn[Q][KJ].origin()), {nj, ni, nchol_per_kp[Q]});
          for (int i = 0; i < ni; i++)
            for (int j = 0; j < nj; j++)
            {
  ...
  if (TGwfn.Node().root())
    for (int Q = 0; Q < nkpts; Q++)
      TGwfn.Cores().all_reduce_in_place_n(raw_pointer_cast(LQKikn[Q].origin()), LQKikn[Q].num_elements(), std::plus<>());
    ...
    for (int Q = 0; Q < (nkpts + 1); Q++)
    {
      for (int K = 0; K < nkpts; K++, nt++)
      {
        if (nt % TG.Node().size() == TG.Node().rank())
        {
          ...
          if (type == COLLINEAR)
          {
            std::fill_n(raw_pointer_cast(LQKank[nq0 + nkpts + 1 + Q][K].origin()),
          ...
          if(Q==Q0) {
            if(type==COLLINEAR) {
              { // Alpha
                ...
                boost::multi::array_ref<SPComplexType,2> Likn(raw_pointer_cast(LQKikn[Q0][K].origin()),
                                                           {nmo_per_kp[K],nmo_per_kp[QKtok2[Q0][K]]*nchol_per_kp[Q0]});
                boost::multi::array_ref<SPComplexType,3> Llbn(raw_pointer_cast(LQKank[nq0+nkpts][K].origin()),
                ...
                boost::multi::array_ref<SPComplexType,3> Lbnl(raw_pointer_cast(LQKank[nq0+nkpts][K].origin()),
              ...
              { // Beta
                ...
                boost::multi::array_ref<SPComplexType,3> Llbn(raw_pointer_cast(LQKank[nq0+2*nkpts+1][K].origin()),
                ...
                boost::multi::array_ref<SPComplexType,3> Lbnl(raw_pointer_cast(LQKank[nq0+2*nkpts+1][K].origin()),
                                                           {nocc_per_kp[nd][QKtok2[Q0][K]],nchol_per_kp[Q0],nmo_per_kp[K]});
                boost::multi::array<SPComplexType,3> Llbn_({nmo_per_kp[K],
                                                            nocc_per_kp[nd][QKtok2[Q0][K]],
                                                            nchol_per_kp[Q0]});
...
// change get_PsiK to cast to the value_type of the result
              ...
              boost::multi::array_ref<SPComplexType,2> Lakn(raw_pointer_cast(LQKank[nq0+Q][K].origin()),
...
// change get_PsiK to cast to the value_type of the result
              ...
              utils::check(Psi.size(0) == nocc_per_kp[nd][nkpts+K], "");
              boost::multi::array_ref<SPComplexType,2> Likn(raw_pointer_cast(LQKikn[Q][K].origin()),
                                                           {nmo_per_kp[K],nmo_per_kp[QKtok2[Q][K]]*nchol_per_kp[Q]});
              boost::multi::array_ref<SPComplexType,2> Lakn(raw_pointer_cast(LQKank[nq0+nkpts+1+Q][K].origin()),
...
// change get_PsiK to cast to the value_type of the result
  ...
  TG.Global().barrier();
  if(TG.Node().root()) {
    TG.Cores().all_reduce_in_place_n(raw_pointer_cast(haj.origin()),
                                     haj.num_elements(),std::plus<>());
    for(int Q=0; Q<LQKank.size(); Q++)
      TG.Cores().all_reduce_in_place_n(raw_pointer_cast(LQKank[Q].origin()),
...
*/
    ...
    for (int Q = 0; Q < nkpts; Q++)
    {
      ...
      if (kminus[Q] == Q)
      {
        ...
        for (int KI = 0; KI < nkpts; KI++)
        {
          ...
          int ni = nmo_per_kp[KI];
          int nj = nmo_per_kp[KJ];
          boost::multi::array_ref<SPComplexType, 3> LQI(raw_pointer_cast(LQKikn_[Q][KI].origin()),
                                                        {ni, nj, nchol_per_kp_[Q]});
          boost::multi::array_ref<SPComplexType, 3> LQJ(raw_pointer_cast(LQKikn_[Q][KJ].origin()),
      ...
      else if (kminus[Q] < Q)
      {
        ...
        for (int KI = 0; KI < nkpts; KI++)
        {
          ...
          int ni = nmo_per_kp[KI];
          int nj = nmo_per_kp[KJ];
          boost::multi::array_ref<SPComplexType, 3> LQm(raw_pointer_cast(LQKikn_[Qm][KI].origin()),
                                                        {ni, nj, nchol_per_kp_[Q]});
          boost::multi::array_ref<SPComplexType, 3> LQ(raw_pointer_cast(LQKikn_[Q][KJ].origin()), {nj, ni, nchol_per_kp_[Q]});
          for (int i = 0; i < ni; i++)
            for (int j = 0; j < nj; j++)
            {
              auto LQ_n(LQ[j][i].origin());
  ...
  for (int KI = 0, n_ = 0; KI < nkpts; KI++)
    for (int KL = 0; KL < nkpts; KL++)
    {
      for (int KK = 0; KK < nkpts; KK++)
      {
        ...
        if ((n_++) % TGwfn.Global().size() == TGwfn.Global().rank())
        {
          boost::multi::array_ref<ComplexType, 2> LKI(raw_pointer_cast(LQKikn[Q][KI].origin()),
                                                      {nmo_max * nmo_max, nchol_per_kp[Q]});
          boost::multi::array_ref<ComplexType, 2> LKL(raw_pointer_cast(LQKikn[Q][KL].origin()),
                                                      {nmo_max * nmo_max, nchol_per_kp[Q]});
          boost::multi::array_ref<ComplexType, 2> LKI_(raw_pointer_cast(LQKikn_[Q][KI].origin()),
                                                       {nmo_max * nmo_max, nchol_per_kp_[Q]});
          boost::multi::array_ref<ComplexType, 2> LKL_(raw_pointer_cast(LQKikn_[Q][KL].origin()),
                                                       {nmo_max * nmo_max, nchol_per_kp_[Q]});
  ...
  for(int KL=0; KL<nkpts; KL++) {
    for(int KK=0; KK<nkpts; KK++)
    {
      int Q = KK2Q[KI][KK];
      int KJ = QKtok2[Q][KL];
      boost::multi::array_ref<ComplexType,2> LKI(raw_pointer_cast(LQKikn[Q][KI].origin()),{nmo_max*nmo_max,nchol_per_kp[Q]});
      boost::multi::array_ref<ComplexType,2> LKL(raw_pointer_cast(LQKikn[Q][KL].origin()),{nmo_max*nmo_max,nchol_per_kp[Q]});
      ma::product(LKI,ma::H(LKL),IJKL);
      for(int i=0; i<nmo_per_kp[0]; i++)
      for(int k=0; k<nmo_per_kp[0]; k++)
      for(int j=0; j<nmo_per_kp[0]; j++)
