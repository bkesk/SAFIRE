...
#ifndef SFQMC_AFQMC_ATOMCENTERED_CORRELATORS_HPP
#define SFQMC_AFQMC_ATOMCENTERED_CORRELATORS_HPP
...
class atomcentered_correlators : public AFQMCInfo
{
  ...
  using shm_stack_Ialloc_type = LocalTGBufferManager::template allocator_t<int>;
  using StaticIVector         = boost::multi::static_array<int, 1, shm_stack_Ialloc_type>;
  using StaticVector          = boost::multi::static_array<ComplexType, 1, shm_stack_alloc_type>;
...
public:
  ...
  {
    app_log(1,"  --  Adding atom-centered correlation functions (atomcentered_correlators). -- \n ");
    std::string orb_file = pt.get<std::string>("filename");
    ...
    if (TG.Node().root())
    {
      if (!dump.open(orb_file, H5F_ACC_RDONLY))
        APP_ABORT(" Error opening orbitals file for atomcentered_correlators estimator. ");
      if (dump.push("AtomicOverlaps", false)<0)
        APP_ABORT(" Error in atomcentered_correlators: Group OrbsR not found.");
      if (!dump.readEntry(orbs, "index_of_orbital_per_site"))
        APP_ABORT(" Error in atomcentered_correlators: Problems reading index_of_orbital_per_site. ");
      nsites = orbs.size() - 1;
      if (nsites < 1)
        APP_ABORT(" Error in atomcentered_correlators: nsites < 1. ");
      if (orbs[0] != 0)
        APP_ABORT(" Error in atomcentered_correlators: orbs[0] != 0. ");
      for (int i = 0; i < nsites; i++)
        if (orbs[i] > orbs[i + 1])
          APP_ABORT(" Error in atomcentered_correlators: orbs[i] > orbs[i+1]. ");
      ...
      if (!dump.readEntry(S_, "Left"))
        APP_ABORT(" Error in atomcentered_correlators: Problems reading Left. ");
      stdCMatrix_ref S2_(raw_pointer_cast(S[1].origin()), {NAO, NMO});
      if (!dump.readEntry(S2_, "Right"))
        APP_ABORT(" Error in atomcentered_correlators: Problems reading Right. ");
      dump.pop();
      dump.close();
      // use faster approach
      // slowwwww!!!!!
    ...
    ns2 = nsites * nsites;
    TG.Node().barrier();
    app_log(1," Number of atomic sites: {}", nsites);
    app_log(1," Number of (pseudo) localized atomic orbitals: {}", NAO);
  ...
  void accumulate(int iav, MatG&& G, MatG_host&& G_host, HostCVec1&& Xw, [[maybe_unused]] bool impsamp)
  {
    ...
    while (iw0 < nw)
    {
      ...
      StaticMatrix devNwI({nwlk, nsites}, 
                        buffer_manager.get_generator().template get_allocator<ComplexType>());
      StaticIVector shapes(iextensions<1u>{2 * nw * nsites * nsites}, 
      ...
      for (int is = 0; is < nsp; ++is)
      {
        ...
        batched_diagonal_sum(make_device_ptr(shapes.data()), Barray.data(), NAO, ComplexType(1.0), Aarray.data(), int(Aarray.size()));
        ...
        batched_ab_ba(make_device_ptr(shapes.data()), Aarray.data(), NAO, Barray.data(), NAO, ComplexType(1.0), Carray.data(),
                      int(Aarray.size()));
        TG.TG_local().barrier();
    ...
    for (int iw = 0; iw < nw; iw++)
    {
      ...
      {
        stdCMatrix_ref DMcc(raw_pointer_cast(DMAverage2D[iav][0].origin()), {nsites, nsites});
        stdCMatrix_ref DMss(raw_pointer_cast(DMAverage2D[iav][1].origin()), {nsites, nsites});
        stdCMatrix_ref DMcs(raw_pointer_cast(DMAverage2D[iav][2].origin()), {nsites, nsites});
        auto X_ = 2.0 * Xw[iw];
        for (int i = 0; i < nsites; i++)
        {
          if (i % TG.TG_local().size() != TG.TG_local().rank())
            continue;
          DMAverage1D[iav][0][i] += X_ * NwI[0][iw][i];
      ...
      else if (walker_type == COLLINEAR)
      {
        ...
        for (int i = 0; i < nsites; i++)
        {
          if (i % TG.TG_local().size() != TG.TG_local().rank())
            continue;
          DMAverage1D[iav][0][i] += X_ * (NwI[0][iw][i] + NwI[1][iw][i]);
          DMAverage1D[iav][1][i] += X_ * (NwI[0][iw][i] - NwI[1][iw][i]);
          DMAverage1D[iav][2][i] += std::real(ComplexType(X_ * (NwI[0][iw][i] - NwI[1][iw][i])));
          for (int j = 0; j < nsites; j++)
          {
...
/*******   Interface for PHMSD-like wfns: Reference + excited configurations  *******/
  ...
  void print(int iblock, hdf_archive& dump, HostCVec&& Wsum)
  {
    ...
    if (TG.TG_local().root())
    {
      ...
      TG.TG_heads().reduce_in_place_n(raw_pointer_cast(DMAverage1D.origin()), DMAverage1D.num_elements(), std::plus<>(), 0);
      ma::scal(ComplexType(1.0 / block_size), DMAverage2D);
      TG.TG_heads().reduce_in_place_n(raw_pointer_cast(DMAverage2D.origin()), DMAverage2D.num_elements(), std::plus<>(), 0);
      if (writer)
      {
        ...
        for (int t = 0; t < 3; ++t)
        {
          ...
          for (int i = 0; i < nave; ++i)
          {
            ...
            stdCVector_ref DMAverage2D_(raw_pointer_cast(DMAverage2D[i][t].origin()), {ns2});
        ...
        for (int t = 0; t < 3; ++t)
        {
          ...
          for (int i = 0; i < nave; ++i)
          {
            ...
            std::string padded_iblock =
                std::string(n_zero - std::to_string(iblock).length(), '0') + std::to_string(iblock);
            stdCVector_ref DMAverage1D_(raw_pointer_cast(DMAverage1D[i][t].origin()), {nsites});
