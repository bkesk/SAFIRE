...
public:
        ...
        denominator(1)
  {
    for(const ptree::value_type &it : pt)
    {
      ...
      if (cname == "onerdm")
      {
        properties_1body.emplace_back(full1rdm(mpi, it.second, walker_type, NMO, 1));
      }
...
//            generalizedFockMatrix(mpi, info, it.second, walker_type, *wfn, 1, block_size));
      ...
      {
       properties.emplace_back(diagonal2rdm<MEM>(mpi, it.second, walker_type, NMO, 1));
      }
      else if (cname == "twordm")
      {
       properties.emplace_back(full2rdm<MEM>(mpi, it.second, walker_type, NMO, 1));
      }
...
//        properties.emplace_back(atomcentered_correlators(mpi, info, it.second, walker_type, 1, block_size));
      ...
      {
        properties.emplace_back(pair_correlator(mpi, it.second, walker_type, NMO, 1));
      }
      else if (cname == "spinspin")
      {
        properties.emplace_back(spinspinobs(mpi, it.second, walker_type, NMO, 1));
      }
    }
