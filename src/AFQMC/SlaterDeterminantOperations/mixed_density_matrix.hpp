                      ...
                      bool herm    = true)
{
  ...
  RUNTIME_CHECK(NMO == B.size(0), "");
  RUNTIME_CHECK(NEL == B.size(1), "");
  RUNTIME_CHECK(NEL == T1.size(0), "");
  RUNTIME_CHECK(B.size(1) == T1.size(1), "");
  if (compact)
  {
    RUNTIME_CHECK(C.size(0) == T1.size(1), "");
    RUNTIME_CHECK(C.size(1) == B.size(0), "");
  }
  else
  {
    RUNTIME_CHECK(T2.size(1) == B.size(0), "");
    RUNTIME_CHECK(T2.size(0) == T1.size(1), "");
    RUNTIME_CHECK(C.size(0) == NMO, "");
    RUNTIME_CHECK(C.size(1) == T2.size(1), "");
  }
                                 ...
                                 bool compact = true)
{
  ...
  RUNTIME_CHECK(hermA.size(0) == TAB.size(0), "");
  RUNTIME_CHECK(B.size(1) == TAB.size(1), "");
  RUNTIME_CHECK(B.size(1) == TNN.size(0), "");
  RUNTIME_CHECK(B.size(1) == TNN.size(1), "");
  RUNTIME_CHECK(hermA.size(0) == QQ0.size(0), "");
  RUNTIME_CHECK(B.size(1) == QQ0.size(1), "");
  if (compact)
  {
    RUNTIME_CHECK(C.size(0) == TNN.size(1), "");
    RUNTIME_CHECK(C.size(1) == B.size(0), "");
  }
  else
  {
    RUNTIME_CHECK(TNM.size(1) == B.size(0), "");
    RUNTIME_CHECK(TNM.size(0) == TNN.size(1), "");
    RUNTIME_CHECK(C.size(0) == hermA.size(1), "");
    RUNTIME_CHECK(C.size(1) == TNM.size(1), "");
  }
  ...
  // TNN = TAB[ref,:]
  using Itype = typename std::pointer_traits<Iptr>::element_type;
  Vector_ref<Itype,Iptr> ivec(ref, iextensions<1u>(NEL));
  ma::copy_select(TAB, TNN, ivec, false);
                             ...
                             bool compact = true)
{
  ...
  RUNTIME_CHECK(A.size(1) == T1.size(0), "");
  RUNTIME_CHECK(B.size(1) == T1.size(1), "");
  if (compact)
  {
    RUNTIME_CHECK(C.size(0) == T1.size(1), "");
  ...
  else
  {
    RUNTIME_CHECK(T2.size(1) == B.size(0), "");
    RUNTIME_CHECK(T2.size(0) == T1.size(1), "");
    RUNTIME_CHECK(C.size(0) == A.size(0), "");
    RUNTIME_CHECK(C.size(1) == T2.size(1), "");
  }
                                  ...
                                  bool compact = true)
{
  ...
  RUNTIME_CHECK(A.size(0) == B.size(0), "");
  RUNTIME_CHECK(A.size(1) == B.size(1), "");
  RUNTIME_CHECK(A.size(1) == U.size(0), ""); // [U] = [NxN]
  RUNTIME_CHECK(A.size(1) == U.size(1), "");
  RUNTIME_CHECK(A.size(1) == VT.size(0), ""); // [V] = [NxN]
  RUNTIME_CHECK(A.size(1) == VT.size(1), "");
  RUNTIME_CHECK(A.size(1) <= (6 * S.size(0) + 1), ""); // [S] = [N+1]
  RUNTIME_CHECK(A.size(1) == UA.size(0), "");          // [UA] = [NxM]
  RUNTIME_CHECK(A.size(0) == UA.size(1), "");
  if (compact)
  {
    RUNTIME_CHECK(C.size(0) == B.size(1), "");
    RUNTIME_CHECK(C.size(1) == B.size(0), "");
  }
  else
  {
    RUNTIME_CHECK(A.size(0) == BV.size(0), ""); // [BV] = [MxN]
    RUNTIME_CHECK(A.size(1) == BV.size(1), "");
    RUNTIME_CHECK(C.size(0) == A.size(0), "");
    RUNTIME_CHECK(C.size(1) == A.size(0), "");
  }
...
/*
    ...
    term_by_term_matrix_vector(ma::TOp_DIV, 0, VT.size(0), VT.size(1), 
                               ma::pointer_dispatch(VT.origin()), VT.stride(0),
                               RType(1.0), ma::pointer_dispatch(S.origin()), 1);
*/
    ma::elementwise(ma::TOp_DIV, 0, S.sliced(0, N), VT); 
...
/*
    term_by_term_matrix_vector(ma::TOp_DIV, 1, BV.size(0), BV.size(1), 
                               ma::pointer_dispatch(BV.origin()), BV.stride(0),
                               RType(1.0), ma::pointer_dispatch(S.origin()), 1);
*/
    ma::elementwise(ma::TOp_DIV, 0, S.sliced(0, N), BV); 
                      ...
                      TBuffer&& WORK)
{
  ...
  int NEL = B.size(1);
  RUNTIME_CHECK(hermA.size(1) == B.size(0), "");
  RUNTIME_CHECK(hermA.size(0) == TMN.size(0), "");
  RUNTIME_CHECK(B.size(1) == TMN.size(1), "");
  RUNTIME_CHECK(B.size(1) == TNN.size(0), "");
  RUNTIME_CHECK(B.size(1) == TNN.size(1), "");
  RUNTIME_CHECK(hermA.size(0) == QQ0.size(0), "");
  RUNTIME_CHECK(B.size(1) == QQ0.size(1), "");
  ...
  using Itype = typename std::pointer_traits<Iptr>::element_type;
  Vector_ref<Itype,Iptr> ivec(ref, iextensions<1u>(NEL));
  ma::copy_select(TMN, TNN, ivec, false);
                                 ...
                                 bool compact = true)
{
  ...
  if (N0 != Nn)
  {
    ...
    // TNN = TAB[ref,:]
    for (int i = 0; i < NEL; i++)
      copy_n(raw_pointer_cast(TAB[*(ref + i)].origin()) + N0, Nn - N0, raw_pointer_cast(TNN[i].origin()) + N0);
  ...
  if (comm.rank() == 0) {
    ...
    if (std::abs(ovlp) == Tp(0.0))
    {
      using Celement = typename std::decay<MatC>::type::element;
      ma::fill(C, Celement(0.0));
      using Delement = typename std::decay<MatD>::type::element;
      ma::fill(QQ0, Delement(0.0));
    }
  }  
  comm.broadcast_n(&ovlp, 1, 0);
  if (std::abs(ovlp) == Tp(0.0))
                        ...
                        bool herm    = true)
{
  ...
  RUNTIME_CHECK((*Bi[0]).size(0) == NMO, "");
  RUNTIME_CHECK((*Bi[0]).size(1) == NEL, "");
  RUNTIME_CHECK(C.size(0) == nbatch, "");
  RUNTIME_CHECK(C.size(2) == NMO, "");
  if (compact)
    RUNTIME_CHECK(C.size(1) == NEL, "");
  else
    RUNTIME_CHECK(C.size(1) == NMO, "");
  RUNTIME_CHECK(ovlp.size() == nbatch, "");
  RUNTIME_CHECK(TNN3D.size(1) == NEL, "");
  RUNTIME_CHECK(TNN3D.size(2) == NEL, "");
  if (not compact)
  {
    RUNTIME_CHECK(TNM3D.size(0) == nbatch, "");
    RUNTIME_CHECK(TNM3D.size(1) == NEL, "");
    RUNTIME_CHECK(TNM3D.size(2) == NMO, "");
  }
  RUNTIME_CHECK(IWORK.num_elements() >= nbatch * (NEL + 1), "");
  RUNTIME_CHECK(TNN3D.stride(1) == NEL, ""); // needed by getriBatched
  ...
  strided_determinant_from_getrf(NEL, ma::pointer_dispatch(Carray[0]), ldC, C.stride(0),
                                 ...
                                 raw_pointer_cast(ovlp.origin()), ovlp.stride(0), nbatch);

  getriBatched(NEL, Carray.data(), ldC, ma::pointer_dispatch(IWORK.origin()), NNarray.data(), ldN,
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  // zero out TNN if determinant is zer
  ma::fill_if_zero(TNN3D, ovlp, typename std::decay_t<Mat>::element(0.0));
...
template<class MatA, class Mat3DB, class Mat3DC, 
         class Mat, class TVec, class IBuffer, class Tp>
void MixedDensityMatrix(MatA const& hermA,
                        Mat3DB&& B,
                        Mat3DC&& C,
                        Tp LogOverlapFactor,
                        TVec&& ovlp,
                        ...
                        bool herm    = true)
{
  static_assert(std::decay_t<MatA>::dimensionality == 2 or
                std::decay_t<MatA>::dimensionality == -2, " MatA::dimensionality == 2/-2");
  static_assert(std::decay_t<Mat3DB>::dimensionality == 3, " Mat3DB::dimensionality == 3");
  static_assert(std::decay_t<Mat3DC>::dimensionality == 3, " Mat3DC::dimensionality == 3");
  static_assert(std::decay_t<TVec>::dimensionality == 1, " TVec::dimensionality == 1");
  static_assert(std::decay_t<Mat>::dimensionality == 3, " Mat::dimensionality == 3");
  static_assert(std::decay_t<IBuffer>::dimensionality == 1, " IBuffer::dimensionality == 1");

  using ma::productStridedBatched;
  using ma::getrfBatched;
  using ma::getriBatched;
  using ma::H;
  using ma::T;
  ...
  RUNTIME_CHECK(B.size(1) == NMO, "");
  RUNTIME_CHECK(B.size(2) == NEL, "");
  RUNTIME_CHECK(C.size(0) == nbatch, "");
  RUNTIME_CHECK(C.size(2) == NMO, "");
  if (compact)
    RUNTIME_CHECK(C.size(1) == NEL, "");
  ...
  std::vector<decltype(ma::pointer_dispatch(C[0].origin()))> Carray;
  std::vector<decltype(ma::pointer_dispatch(TNN3D[0].origin()))> NNarray;
  Carray.reserve(nbatch);
  NNarray.reserve(nbatch);
  for (int i = 0; i < nbatch; i++)
  {
    NNarray.emplace_back(ma::pointer_dispatch(TNN3D[i].origin()));
    Carray.emplace_back(ma::pointer_dispatch(C[i].origin()));
  }

  // Using C tensor as temporary storage.
  //T(conj(A))*B
  if (herm)
    ma::productStridedBatched(hermA, B, C(C.extension(0), {0, NEL}, {0, NEL}));
  else
    ma::productStridedBatched(ma::H(hermA), B, C(C.extension(0), {0, NEL}, {0, NEL}));

  // Invert Ct into TNN3D
  getrfBatched(NEL, Carray.data(), C.stride(1), ma::pointer_dispatch(IWORK.origin()),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  using ma::strided_determinant_from_getrf;
  strided_determinant_from_getrf(NEL, ma::pointer_dispatch(Carray[0]), C.stride(1), C.stride(0),
            ma::pointer_dispatch(IWORK.origin()), NEL, LogOverlapFactor, 
            raw_pointer_cast(ovlp.origin()), ovlp.stride(0), nbatch);

  getriBatched(NEL, Carray.data(), C.stride(1), ma::pointer_dispatch(IWORK.origin()), 
               NNarray.data(), TNN3D.stride(1),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);
  ...
  if (compact)
  {
    ma::productStridedBatched(ma::T(TNN3D), ma::T(B), C);  
  }
  else
  {
    if (herm)
    {
      // T2 = T(T1) * T(B)
      ma::productStridedBatched(ma::T(TNN3D), ma::T(B), TNM3D);  

      // C = conj(A) * T2
      ma::productStridedBatched(ma::T(hermA), TNM3D, C);  
    }
    else
    {
      // T2 = T1 * H(A)
      ...
      // T2 = T(T1) * T(B)
      // C = T( B * T2) = T(T2) * T(B)
      ma::productStridedBatched(ma::T(TNM3D), ma::T(B), C);
    }
  }
}
                     ...
                     bool herm    = true)
{
  ...
  RUNTIME_CHECK((*Right[0]).size(0) == NMO, "");
  RUNTIME_CHECK((*Right[0]).size(1) == NEL, "");
  RUNTIME_CHECK(G.size() == nbatch, "");
  RUNTIME_CHECK((*G[0]).size(1) == NMO, "");
  if (compact)
    RUNTIME_CHECK((*G[0]).size(0) == NEL, "");
  else
    RUNTIME_CHECK((*G[0]).size(0) == NMO, "");
  RUNTIME_CHECK(ovlp.size() == nbatch, "");
  RUNTIME_CHECK(TNN3D.size(1) == NEL, "");
  RUNTIME_CHECK(TNN3D.size(2) == NEL, "");
  if (not compact)
  ...
  for (int i = 0; i < nbatch; i++)
  {
    RUNTIME_CHECK((*Right[i]).stride(0) == ldR, "");
    RUNTIME_CHECK((*Left[i]).stride(0) == ldL, "");
    NNarray.emplace_back(TNN3D[i].origin());
             ...
             bool herm = true)
{
  ...
  int nbatch = Bi.size();
  RUNTIME_CHECK(hermA.size() >= nbatch, "");
  int NMO = (herm ? (*hermA[0]).size(1) : (*hermA[0]).size(0));
  int NEL = (herm ? (*hermA[0]).size(0) : (*hermA[0]).size(1));
...
template<class MatA, class Mat3DB, class Mat, class TVec, class IBuffer, class Tp>
void Overlap(MatA const& hermA,
             Mat3DB&& B,
             Tp LogOverlapFactor,
             TVec&& ovlp,
             Mat&& TNN3D,
             ...
             bool herm = true)
{
  ...
  // T(conj(A))*B
  if (herm)
    ma::productStridedBatched(hermA, B, TNN3D);
  else
    ma::productStridedBatched(ma::H(hermA), B, TNN3D);

  // T1 = T1^(-1)
  // Invert
  getrfBatched(NEL, NNarray.data(), TNN3D.stride(1), 
               IWORK.origin(), IWORK.origin() + nbatch * NEL, nbatch);

  using ma::strided_determinant_from_getrf;
  strided_determinant_from_getrf(NEL, NNarray[0], TNN3D.stride(1), TNN3D.stride(0), 
        IWORK.origin(), NEL, LogOverlapFactor, raw_pointer_cast(ovlp.origin()), 
        ovlp.stride(0), nbatch);
}

template<class Tp, class Iptr, class MatA, class Mat3DB, class Mat3DC,
         class TVec, class Mat1, class Mat2, class Mat3, class IBuffer>
void MixedDensityMatrixFromConfiguration(MatA const& hermA,
                                       Mat3DB const& B,
                                       Mat3DC&& C,
                                       Tp LogOverlapFactor,
                                       TVec&& ovlp, 
                                       ...
                                       bool compact = true)
{
  ...
  static_assert(std::decay_t<Mat3DC>::dimensionality == 3, " Mat3DC::dimensionality == 3");
  static_assert(std::decay_t<TVec>::dimensionality == 1, " TVec::dimensionality == 1");
  static_assert(std::decay_t<Mat1>::dimensionality == 3, " Mat::dimensionality == 3");
  static_assert(std::decay_t<Mat2>::dimensionality == 3, " Mat::dimensionality == 3");
  static_assert(std::decay_t<Mat3>::dimensionality == 3, " Mat::dimensionality == 3");
  static_assert(std::decay_t<IBuffer>::dimensionality == 1, " IBuffer::dimensionality == 1");
  ...
  RUNTIME_CHECK(hermA.size(0) == TAB.size(1), "");
  RUNTIME_CHECK(B.size(2) == TAB.size(2), "");
  RUNTIME_CHECK(B.size(2) == TNN.size(1), "");
  RUNTIME_CHECK(B.size(2) == TNN.size(2), "");
  if (compact)
  {
    RUNTIME_CHECK(C.size(1) == B.size(2), "");
    RUNTIME_CHECK(C.size(2) == B.size(1), "");
  }
  else
  {
    RUNTIME_CHECK(TNM.size(2) == B.size(1), "");
    RUNTIME_CHECK(TNM.size(1) == TNN.size(2), "");
    RUNTIME_CHECK(C.size(1) == TNM.size(2), "");
    RUNTIME_CHECK(C.size(2) == C.size(1), "");
  }

  std::vector<decltype(ma::pointer_dispatch(TAB[0].origin()))> ABarray;
  std::vector<decltype(ma::pointer_dispatch(TNN[0].origin()))> NNarray;
  ABarray.reserve(nbatch);
  NNarray.reserve(nbatch);
  for (int i = 0; i < nbatch; i++)
  { 
    ABarray.emplace_back(ma::pointer_dispatch(TAB[i].origin()));
    NNarray.emplace_back(ma::pointer_dispatch(TNN[i].origin()));
  }

  // TAB = herm(A)*B  
  ma::productStridedBatched(hermA, B, TAB); 
  ...
  // Invert TNN into TAB 
  getrfBatched(NEL, NNarray.data(), TNN.stride(1), ma::pointer_dispatch(IWORK.origin()),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  using ma::strided_determinant_from_getrf;
  strided_determinant_from_getrf(NEL, ma::pointer_dispatch(NNarray[0]), TNN.stride(1), 
            TNN.stride(0), ma::pointer_dispatch(IWORK.origin()), NEL, LogOverlapFactor,
            raw_pointer_cast(ovlp.origin()), ovlp.stride(0), nbatch);

  getriBatched(NEL, NNarray.data(), TNN.stride(1), ma::pointer_dispatch(IWORK.origin()),
               ABarray.data(), TAB.stride(1),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  // zero out TNN if determinant is zer
  ma::fill_if_zero(TAB, ovlp, typename std::decay_t<Mat2>::element(0.0));

  if (compact)
  {
    // C = T(TAB) * T(B)
    ma::productStridedBatched(T(TAB({0,nbatch},{0,NEL},{0,NEL})), T(B), std::forward<Mat3DC>(C));
  }
  else
  {
    // TNM = T(TAB) * T(B)
    ma::productStridedBatched(T(TAB({0,nbatch},{0,NEL},{0,NEL})), T(B), std::forward<Mat3>(TNM));

    // C = conj(A) * TNM
    ma::productStridedBatched(T(hermA), TNM, std::forward<Mat3DC>(C));
  }
}
...
template<class Tp,
         ...
         class Mat3DE,
         class IBuffer>
void OverlapForWoodbury(MatA const& hermA,
                      Mat3DB const& B,
                      Tp LogOverlapFactor,
                      TVec&& ovlp,  
                      Mat3DC&& QQ0,
                      ...
                      IBuffer&& IWORK)
{
  ...
  static_assert(std::decay_t<Mat3DC>::dimensionality == 3, " Mat3DC::dimensionality == 3");
  static_assert(std::decay_t<TVec>::dimensionality == 1, " TVec::dimensionality == 1");
  static_assert(std::decay_t<Mat3DD>::dimensionality == 3, " Mat3DD::dimensionality == 3");
  static_assert(std::decay_t<Mat3DE>::dimensionality == 3, " Mat3DE::dimensionality == 3");
  static_assert(std::decay_t<IBuffer>::dimensionality == 1, " IBuffer::dimensionality == 1");
  ...
  int NEL = B.size(2);
  RUNTIME_CHECK(hermA.size(1) == B.size(1), "");
  RUNTIME_CHECK(hermA.size(0) == TMN.size(1), "");
  RUNTIME_CHECK(B.size(2) == TMN.size(2), "");
  RUNTIME_CHECK(B.size(2) == TNN.size(1), "");
  RUNTIME_CHECK(B.size(2) == TNN.size(2), "");
  RUNTIME_CHECK(hermA.size(0) == QQ0.size(1), "");
  RUNTIME_CHECK(B.size(2) == QQ0.size(2), "");
  RUNTIME_CHECK(B.size(0) == QQ0.size(0), "");
  RUNTIME_CHECK(B.size(0) == TNN.size(0), "");
  RUNTIME_CHECK(B.size(0) == TMN.size(0), "");

  using ma::productStridedBatched;
  using ma::getrfBatched;
  using ma::getriBatched;

  std::vector<decltype(ma::pointer_dispatch(QQ0[0].origin()))> QQarray;
  std::vector<decltype(ma::pointer_dispatch(TNN[0].origin()))> NNarray;
  QQarray.reserve(nbatch);
  NNarray.reserve(nbatch);
  for (int i = 0; i < nbatch; i++)
  {
    QQarray.emplace_back(ma::pointer_dispatch(QQ0[i].origin()));
    NNarray.emplace_back(ma::pointer_dispatch(TNN[i].origin()));
  }

  // TMN = herm(A)*B  
  ma::productStridedBatched(hermA, B, std::forward<Mat3DC>(TMN));

  // TNN = QQ0[ref,:]
  using Itype = typename std::pointer_traits<Iptr>::element_type;
  Vector_ref<Itype,Iptr> ivec(ref, iextensions<1u>(NEL));
  ma::copy_select(TMN, QQ0({0,nbatch},{0,NEL},{0,NEL}), ivec, false);

  // Invert QQ into TNN 
  getrfBatched(NEL, QQarray.data(), QQ0.stride(1), ma::pointer_dispatch(IWORK.origin()),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  using ma::strided_determinant_from_getrf;
  strided_determinant_from_getrf(NEL, ma::pointer_dispatch(QQarray[0]), QQ0.stride(1),
            QQ0.stride(0), ma::pointer_dispatch(IWORK.origin()), NEL, LogOverlapFactor,
            raw_pointer_cast(ovlp.origin()), ovlp.stride(0), nbatch);

  getriBatched(NEL, QQarray.data(), QQ0.stride(1), ma::pointer_dispatch(IWORK.origin()),
               NNarray.data(), TNN.stride(1),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  // zero out TNN if determinant is zer
  ma::fill_if_zero(TNN, ovlp, typename std::decay_t<Mat3DD>::element(0.0));

  // QQ0 = TMN * inv(TNN)
  ma::productStridedBatched(TMN, TNN, std::forward<Mat3DC>(QQ0));
...
template<class Tp,
         ...
         class Mat3DQ,
         class IBuffer>
void MixedDensityMatrixForWoodbury(MatA const& hermA,
                      Mat3DB const& B,
                      Mat3DC&& C,
                      Tp LogOverlapFactor,
                      TVec&& ovlp,  
                      ...
                      bool compact)
{
  ...
  static_assert(std::decay_t<Mat3DD>::dimensionality == 3, " Mat3DD::dimensionality == 3");
  static_assert(std::decay_t<Mat3DE>::dimensionality == 3, " Mat3DE::dimensionality == 3");
  static_assert(std::decay_t<Mat3DF>::dimensionality == 3, " Mat3DF::dimensionality == 3");
  static_assert(std::decay_t<Mat3DQ>::dimensionality == 3, " Mat3DQ::dimensionality == 3");
  static_assert(std::decay_t<IBuffer>::dimensionality == 1, " IBuffer::dimensionality == 1");
  ...
  RUNTIME_CHECK(hermA.size(1) == M, "");
  RUNTIME_CHECK(C.size(0) == nbatch, "");
  RUNTIME_CHECK(TNN.size(0) == nbatch, "");
  RUNTIME_CHECK(TNN.size(1) == NEL, "");
  RUNTIME_CHECK(TNN.size(2) == NEL, "");
  RUNTIME_CHECK(QQ0.size(0) == nbatch, "");
  RUNTIME_CHECK(QQ0.size(1) == NACT, "");
  RUNTIME_CHECK(QQ0.size(2) == NEL, "");
  RUNTIME_CHECK(TAB.size(0) == nbatch, "");
  RUNTIME_CHECK(TAB.size(1) == NACT, "");
  RUNTIME_CHECK(TAB.size(2) == NEL, "");
  RUNTIME_CHECK(C.size(2) == M, "");
  if(compact) {
    RUNTIME_CHECK(C.size(1) == NEL, "");
  } else {
    RUNTIME_CHECK(C.size(1) == M, "");
    RUNTIME_CHECK(TNM.size(0) == nbatch, "");
    RUNTIME_CHECK(TNM.size(1) == NEL, "");
    RUNTIME_CHECK(TNM.size(2) == M, "");
  }
  ...
  // TAB = herm(A)*B  
  ma::productStridedBatched(hermA, B, std::forward<Mat3DE>(TAB));

  // TNN = QQ0[ref,:]
  using Itype = typename std::pointer_traits<Iptr>::element_type;
  Vector_ref<Itype,Iptr> ivec(ref, iextensions<1u>(NEL));
  ma::copy_select(TAB, QQ0({0,nbatch},{0,NEL},{0,NEL}), ivec, false);

  // Invert QQ into TNN 
  getrfBatched(NEL, QQarray.data(), int(QQ0.stride(1)), ma::pointer_dispatch(IWORK.origin()),
               ma::pointer_dispatch(IWORK.origin()) + nbatch * NEL, nbatch);

  using ma::strided_determinant_from_getrf;
  strided_determinant_from_getrf(NEL, QQarray[0], QQ0.stride(1),
            QQ0.stride(0), ma::pointer_dispatch(IWORK.origin()), NEL, LogOverlapFactor,
            raw_pointer_cast(ovlp.origin()), ovlp.stride(0), nbatch);
  ...
  // QQ0 = TAB * inv(TNN)
  ma::productStridedBatched(TAB, TNN, std::forward<Mat3DQ>(QQ0));

  if (compact)
  {
    // C = T(TNN) * T(B)
    ma::productStridedBatched(T(TNN), T(B), std::forward<Mat3DC>(C));
  }
  else
  {
    // TNM = T(TNN) * T(B)
    ma::productStridedBatched(T(TNN), T(B), std::forward<Mat3DF>(TNM));
...
template<class Mat3DA, class Mat3DB, class Mat, class TVec, class IBuffer, class Tp,
         typename = void>
void Overlap(Mat3DA&& A,
             Mat3DB&& B,
             Tp LogOverlapFactor,
             TVec&& ovlp,
             Mat&& TNN3D,
             IBuffer&& IWORK)
{
  static_assert(std::decay_t<Mat3DA>::dimensionality == 3, "Mat3DA::dimensionality == 3");  
  static_assert(std::decay_t<Mat3DB>::dimensionality == 3, "Mat3DB::dimensionality == 3");
  static_assert(std::decay_t<TVec>::dimensionality == 1, " TVec::dimensionality == 1");
  static_assert(std::decay_t<Mat>::dimensionality == 3, "Mat::dimensionality == 3");
  ...
  // T(conj(A))*B
  ma::productStridedBatched(ma::H(A), B, TNN3D);

  // T1 = T1^(-1)
  // Invert
  ma::getrfBatched(NEL, NNarray.data(), TNN3D.stride(1), 
               IWORK.origin(), IWORK.origin() + nbatch * NEL, nbatch);
...
template<class Mat3DA, class Mat3DB, class Mat3DC, 
         class Mat, class TVec, class IBuffer, class Tp>
void DensityMatrix(Mat3DA&& A,
                   Mat3DB&& B,
                   Mat3DC&& C,
                   Tp LogOverlapFactor,
                   TVec&& ovlp,
                   ...
                   bool compact )
{
  ...
  // Using C tensor as temporary storage.
  //T(conj(A))*B
  ma::productStridedBatched(ma::H(A), B, C(C.extension(0), {0, NEL}, {0, NEL}));
  ...
  if(compact) {
    ...
    // T2 = T1 * H(A)
    ma::productStridedBatched(TNN3D, ma::H(A), TNM3D);  
