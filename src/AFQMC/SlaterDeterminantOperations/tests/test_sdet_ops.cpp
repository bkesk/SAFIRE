...
/*
  ...
  //SECTION("batched_density_matrix")
  {
    ...
    SDet.BatchedDensityMatrix(RA, RB, Gwv, log_ovlp, ovlp, false);
...
TEST_CASE("SDetOps_complex_mpi3", "[sdet_ops]")
{
  ...
  array_ref G(raw_pointer_cast(SMbuff.origin()), {NMO, NMO});
  array_ref Gc(raw_pointer_cast(SMbuff.origin()) + NMO * NMO, {NEL, NMO});
  ...
  array_ref G2(raw_pointer_cast(SMbuff2.origin()), {NMO, NMO});
  array_ref Gc2(raw_pointer_cast(SMbuff2.origin()) + NMO * NMO, {NEL, NMO});
