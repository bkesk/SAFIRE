...
{
  RUNTIME_CHECK(locV.dimensionality == 2, "");
  RUNTIME_CHECK(Gc.size(1) == Gcloc.size(1), "");
  RUNTIME_CHECK(Gc.size(0) == Gcloc.size(0), "");
  RUNTIME_CHECK(Gc.size(0) == Vakbl.size(0), "");
  RUNTIME_CHECK(Gc.size(0) == Vakbl.size(1), "");
...
inline void calculate_energy(EMat&& locV, const MatA& Gc, MatB&& Gcloc, const SpMat& Vakbl)
{
  ...
  RUNTIME_CHECK(locV.dimensionality == 2, "");
  RUNTIME_CHECK(Gc.size(1) == Gcloc.size(1), "");
  RUNTIME_CHECK(Vakbl.size(0) == Gcloc.size(0), "");
  RUNTIME_CHECK(Gc.size(0) == Vakbl.size(1), "");
