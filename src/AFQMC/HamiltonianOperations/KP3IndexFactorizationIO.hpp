...
#ifndef SFQMC_AFQMC_KP3INDEXIO_HPP
#define SFQMC_AFQMC_KP3INDEXIO_HPP
                                                       ...
                                                       RealType cutv2)
{
  ...
  if (TGwfn.Global().root())
  {
    ...
    if (dump.push("KP3IndexFactorization", false)<0)
    {
    ...
    if (type == COLLINEAR)
    {
      for (int i = 0; i < ndet; i++)
      {
        if (not get_nocc_per_kp(nmo_per_kp, PsiT[2 * i], nocc_per_kp[i]({0, nkpts})))
        {
          ...
          app_error(" Only wavefunctions in block-diagonal form are accepted. ");
          APP_ABORT("");
        }
        ...
        if (nt % TGwfn.Global().size() == TGwfn.Global().rank())
        {
          ...
          if (type == COLLINEAR)
          {
            { // Alpha
              ...
              RUNTIME_CHECK(Psi.shape()[0] == nocc_per_kp[nd][K], "");
