...
#ifndef SFQMC_AFQMC_SPARSETENSORIO_HPP
#define SFQMC_AFQMC_SPARSETENSORIO_HPP
                                      ...
                                      RealType cutv2)
{
  ...
  using T1_mpi3_csr_matrix = ma::sparse::csr_matrix<SpT1, int, std::size_t, shared_allocator<SpT1>, ma::sparse::is_root>;
  using T2_mpi3_csr_matrix = ma::sparse::csr_matrix<SpT2, int, std::size_t, shared_allocator<SpT2>, ma::sparse::is_root>;
  ...
  if (dump.push("SparseTensor", false)<0)
  {
  ...
  std::vector<T1_mpi3_csr_matrix> V2;
  V2.reserve(ndet);
  for (int i = 0; i < ndet; i++)
  {
    if (dump.push(std::string("HalfRotatedHijkl_") + std::to_string(i), false)<0)
    {
      app_error(" Error in loadSparseTensor: Group HalfRotatedHijkl_{} not found", i);
      APP_ABORT("");
    }
    V2.emplace_back(
        csr_hdf5::unstructured_distributed_CSR_from_HDF<T1_mpi3_csr_matrix>(dump, TGwfn, V2_nrows, V2_ncols, cutv2));
    dump.pop();
  }

  // read Cholesky matrix
  if (dump.push(std::string("SparseCholeskyMatrix"), false)<0)
  {
    app_error(" Error in loadSparseTensor: Group SparseCholeskyMatrix not found. ");
    APP_ABORT("");
  }
  SpVType_mpi3_csr_matrix Spvn(
      csr_hdf5::column_distributed_CSR_from_HDF<SpVType_mpi3_csr_matrix>(dump, TGprop, Spvn_nrows, Spvn_ncols, cutv2));
  dump.pop();
  ...
  // setup views
  using matrix_view = typename T1_mpi3_csr_matrix::template matrix_view<int>;
  ...
  if (ndet == 1)
  {
    std::vector<T2_mpi3_csr_matrix> SpvnT;
    // MAM: chech that T2 is SpComplexType
    //std::vector<SpCType_mpi3_csr_matrix> SpvnT;
    using matrix_view = typename T2_mpi3_csr_matrix::template matrix_view<int>;
  ...
  else
  {
    ...
    std::vector<T2_mpi3_csr_matrix> SpvnT;
    //std::vector<SpVType_mpi3_csr_matrix> SpvnT;
    using matrix_view = typename T2_mpi3_csr_matrix::template matrix_view<int>;
    std::vector<matrix_view> SpvnTview;
    SpvnT.emplace_back(csr::shm::transpose<T2_mpi3_csr_matrix>(Spvn));
    SpvnTview.emplace_back(csr::shm::local_balanced_partition(SpvnT[0], TGprop));
