...
#ifndef SFQMC_HDF5_ARCHIVE_H
#define SFQMC_HDF5_ARCHIVE_H
...
public:
   ...
   */
  template<class Comm = boost::mpi3::communicator>
  ...
  /** read attribute from the group aname and return status
   * use read() for inbuilt error checking
   * @return true if successful
   */
  template<typename T>
  bool readAttributeEntry(T& data, const std::string& aname)
  {
    if (Mode[NOIO])
      return true;
    hid_t p = group_id.empty() ? file_id : group_id.top();
    h5data_proxy<T> e(data);
    return e.read_attribute(p, aname);
  }
   ...
   */
  template<typename T>
  void readAttribute(T& data, const std::string& aname)
  {
    if (!readAttributeEntry(data, aname))
    {
      throw std::runtime_error("HDF5 read failure in hdf_archive::readAttribute " + aname);
    }
  }
  ...
  inline void unlink(const std::string& aname)
  {
    ...
    utils::check(status >= 0, "HDF5 unlink failure in hdf_archive::unlink " + aname);
  }
};
