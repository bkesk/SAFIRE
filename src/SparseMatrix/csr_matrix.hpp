...
{
public:
  using value_type  = typename std::pointer_traits<ValTypePtr>::element_type; 
  using element     = typename std::pointer_traits<ValTypePtr>::element_type; 
  using element_type     = typename std::pointer_traits<ValTypePtr>::element_type; 
  using element_ptr = ValTypePtr;
  using index_type  = typename std::pointer_traits<IndxTypePtr>::element_type;
  using int_type    = typename std::pointer_traits<IntTypePtr>::element_type; 
...
public:
  ...
  {
    RUNTIME_CHECK(d == integer_type{0} || d == integer_type{1}, "");
...
// Mainly used for sub matrices with a column range
...
public:
  ...
  csr_matrix_view(std::tuple<size_type, size_type> const& arr,
      ...
      : base(arr, local, global, 0, __data_, __jdata_, IntTypePtr(nullptr), IntTypePtr(nullptr)),
        ...
        ptre(std::move(vb))
  {
    RUNTIME_CHECK(ptrb.size() == base::size1_ + 1, "");
    RUNTIME_CHECK(ptre.size() == base::size1_, "");
...
//         class IntType_alloc  = typename ValType_alloc::template rebind<IntType>::other>
         class IndxType_alloc = typename std::allocator_traits<ValType_alloc>::template rebind_alloc<IndxType>,
         class IntType_alloc  = typename std::allocator_traits<ValType_alloc>::template rebind_alloc<IntType>>
class ucsr_matrix : public csr_matrix_ref<typename std::allocator_traits<ValType_alloc>::pointer,
                                          typename std::allocator_traits<IndxType_alloc>::pointer,
                                          typename std::allocator_traits<IntType_alloc>::pointer>
{
public:
  using value_type  = ValType;
  using element     = ValType;
  using element_ptr = typename std::allocator_traits<ValType_alloc>::pointer;
...
protected:
  ...
  using IndxTypePtr = typename std::allocator_traits<IndxType_alloc>::pointer;
  using IntTypePtr  = typename std::allocator_traits<IntType_alloc>::pointer;
  using const_ValTypePtr  = typename std::allocator_traits<ValType_alloc>::const_pointer;
  using const_IndxTypePtr = typename std::allocator_traits<IndxType_alloc>::const_pointer;
  using const_IntTypePtr  = typename std::allocator_traits<IntType_alloc>::const_pointer;
...
public:
  ...
  template<typename integer_type = size_type,
	   typename = std::enable_if_t<std::is_integral_v<integer_type>>>
  ...
  template<typename integer_type = size_type, class IVec,
	   typename = std::enable_if_t<not std::is_integral_v<IVec>>>
  ucsr_matrix(std::tuple<size_type, size_type> const& arr,
              std::tuple<size_type, size_type> const& global,
              //std::vector<integer_type> const& nnzpr = std::vector<integer_type>(0),
              IVec const& nnzpr = IVec(0),
  ...
  {
    using std::get;
    RUNTIME_CHECK(get<0>(indices) >= 0, "");
    RUNTIME_CHECK(get<0>(indices) < base::size1_, "");
    if (base::pointers_end_[get<0>(indices)] < base::pointers_begin_[get<0>(indices) + 1])
    {
      ::new (static_cast<void*>(raw_pointer_cast(base::data_ + base::pointers_end_[get<0>(indices)])))
          value_type(std::forward<Args>(args)...);
      ::new (static_cast<void*>(raw_pointer_cast(base::jdata_ + base::pointers_end_[get<0>(indices)])))
...
public:
  ...
  {}
  template<typename integer_type = size_type, class IVec,
	   typename = typename std::enable_if_t<not std::is_integral_v<IVec>>>
  csr_matrix(std::tuple<size_type, size_type> const& arr,
             std::tuple<size_type, size_type> const& global,
             IVec const& nnzpr = IVec(0),
             ValType_alloc alloc              = ValType_alloc{})
  ...
  csr_matrix(this_t const& csr, ValType_alloc const& alloc)
      : base(std::tuple<size_type, size_type>{csr.size1_, csr.size2_},
             std::tuple<size_type, size_type>{csr.global_origin1_, csr.global_origin2_},
             0,
             alloc)
        ...
        base(std::tuple<size_type, size_type>{0, 0}, std::tuple<size_type, size_type>{0, 0}, 0, alloc)
  {
    ...
    using sfqmc::afqmc::copy_n_cast;
    copy_n_cast(ma::pointer_dispatch(csr.non_zero_values_data()), base::capacity_, base::data_);
    copy_n_cast(ma::pointer_dispatch(csr.non_zero_indices2_data()), base::capacity_, base::jdata_);
    copy_n_cast(ma::pointer_dispatch(csr.pointers_begin()), base::size1_ + 1, base::pointers_begin_);
    copy_n_cast(ma::pointer_dispatch(csr.pointers_end()), base::size1_, base::pointers_end_);
  }

  // specialized copy constructor which allows you to change ValType
  // but must keep consistent allocators (otherwise copy may fail) 
  template<class ValType_,
           class ValType_alloc_,
           class IsRoot_,
           typename = std::enable_if_t<not std::is_same<ValType_, ValType>::value>>
  csr_matrix(
      csr_matrix<ValType_, IndxType, IntType, ValType_alloc_, IsRoot_, IndxType_alloc, IntType_alloc> const& csr,
      ValType_alloc const& alloc = {})
      :
        base(std::tuple<size_type, size_type>{0, 0}, std::tuple<size_type, size_type>{0, 0}, 0, alloc)
  {
    ...
    using sfqmc::afqmc::copy_n_cast;
    copy_n_cast(ma::pointer_dispatch(csr.non_zero_values_data()), base::capacity_, base::data_);
    copy_n(ma::pointer_dispatch(csr.non_zero_indices2_data()), base::capacity_, base::jdata_);
    copy_n(ma::pointer_dispatch(csr.pointers_begin()), base::size1_ + 1, base::pointers_begin_);
    copy_n(ma::pointer_dispatch(csr.pointers_end()), base::size1_, base::pointers_end_);
  }
      ...
      csr_matrix<ValType, IndxType, IntType, ValType_alloc_, IsRoot_, IndxType_alloc_, IntType_alloc_> const& csr)
  {
    ...
    base::pointers_end_   = base::Palloc_.allocate(base::size1_);
    using std::copy_n;
    copy_n(ma::pointer_dispatch(csr.non_zero_values_data()), base::capacity_, base::data_);
    copy_n(ma::pointer_dispatch(csr.non_zero_indices2_data()), base::capacity_, base::jdata_);
    copy_n(ma::pointer_dispatch(csr.pointers_begin()), base::size1_ + 1, base::pointers_begin_);
    copy_n(ma::pointer_dispatch(csr.pointers_end()), base::size1_, base::pointers_end_);
    return *this;
  ...
  csr_matrix& operator=(ucsr_matrix<ValType, IndxType, IntType, ValType_alloc, IsRoot> const& other)
  {
    ...
    if (base::size1_ == 0 || base::capacity_ == 0)
      return *this;
    using sfqmc::make_paired_iterator;
    ...
    for (size_type p = 0; p < base::size1_; p++)
    {
      if (p % static_cast<size_type>(r.size()) == static_cast<size_type>(r.rank()))
      {
        ...
        std::sort(make_paired_iterator(raw_pointer_cast(base::jdata_ + i1), raw_pointer_cast(base::data_ + i1)),
                  make_paired_iterator(raw_pointer_cast(base::jdata_ + i2), raw_pointer_cast(base::data_ + i2)),
  ...
  csr_matrix(ucsr_matrix<ValType, IndxType, IntType, ValType_alloc, IsRoot> const& ucsr)
      : csr_matrix(tp_ul_ul{0, 0}, tp_ul_ul{0, 0}, 0, ucsr.get_allocator())
  {
    *this = ucsr;
  }
  ...
  csr_matrix(this_t&& other, ValType_alloc const& alloc) : csr_matrix(tp_ul_ul{0, 0}, tp_ul_ul{0, 0}, 0, alloc)
  {
    *this = std::move(other);
  }
  csr_matrix(ucsr_matrix<ValType, IndxType, IntType, ValType_alloc, IsRoot> const& ucsr,
	     ValType_alloc const& alloc)
      : csr_matrix(tp_ul_ul{0, 0}, tp_ul_ul{0, 0}, 0, alloc)
  {
    *this = ucsr;
  }
  csr_matrix(ucsr_matrix<ValType, IndxType, IntType, ValType_alloc, IsRoot>&& ucsr,
  ...
  void emplace(Pair&& indices, Args&&... args)
  {
    ...
    RUNTIME_CHECK(get<0>(indices) >= 0, "");
    RUNTIME_CHECK(get<0>(indices) < base::size1_, "");
    auto loc = std::lower_bound(raw_pointer_cast(base::jdata_ + base::pointers_begin_[get<0>(indices)]), 
                                  raw_pointer_cast(base::jdata_ + base::pointers_end_[get<0>(indices)]), get<1>(indices));
    size_type disp =
        std::distance(raw_pointer_cast(base::jdata_ + base::pointers_begin_[get<0>(indices)]), raw_pointer_cast(loc));  
    size_type disp_ = std::distance(raw_pointer_cast(loc), raw_pointer_cast(base::jdata_ + base::pointers_end_[get<0>(indices)]));
    
    if (disp_ > 0 && *loc == get<1>(indices))
    { 
      // value exists, construct in place
      ::new (static_cast<void*>(raw_pointer_cast(base::data_ + base::pointers_begin_[get<0>(indices)] + disp)))         
          value_type(std::forward<Args>(args)...);
    }
    else
    { 
      ...
      { 
        ...
        if (base::pointers_end_[get<0>(indices)] >= base::pointers_begin_[get<0>(indices) + 1])
          throw std::out_of_range("row size exceeded the maximum");
        std::move_backward(raw_pointer_cast(base::data_ + base::pointers_begin_[get<0>(indices)] + disp),
                           raw_pointer_cast(base::data_ + base::pointers_end_[get<0>(indices)]), 
                           raw_pointer_cast(base::data_ + base::pointers_end_[get<0>(indices)] + 1));
        std::move_backward(raw_pointer_cast(base::jdata_ + base::pointers_begin_[get<0>(indices)] + disp),
                           raw_pointer_cast(base::jdata_ + base::pointers_end_[get<0>(indices)]), 
                           raw_pointer_cast(base::jdata_ + base::pointers_end_[get<0>(indices)] + 1));
      }
      ++base::pointers_end_[get<0>(indices)];
      ::new (static_cast<void*>(raw_pointer_cast(base::data_ + base::pointers_begin_[get<0>(indices)] + disp)))         
          value_type(std::forward<Args>(args)...);
      ::new (static_cast<void*>(raw_pointer_cast(base::jdata_ + base::pointers_begin_[get<0>(indices)] + disp)))        
          index_type(get<1>(indices));
    }
  }
...
/*
// MAM: This will be a problem in GPU code, handle references properly if you need this
  ...
  value_type& get(Pair&& indices)
  {
    ...
    if (not (disp_ > 0 && *loc == get<1>(indices)) )
    {
      // new value, shift back 
      if (disp_ > 0)
      {
        // check if row is full
        if (base::pointers_end_[get<0>(indices)] >= base::pointers_begin_[get<0>(indices) + 1])
    ...
    return *(base::data_ + base::pointers_begin_[get<0>(indices)] + disp);
  }
*/
  template<class Pair = std::array<IndxType, 2>>
  value_type get(Pair&& indices)
  ...
  { 
    ...
    {
      // value exists
      *(base::data_ + base::pointers_begin_[get<0>(indices)] + disp) += static_cast<value_type>(v_);
    }
    else
    { 
      // check if row is full
  ...
  void remove_empty_spaces()
  {
    ...
    if (r.root())
    {
      for (size_type i = 0; i < base::size1_ - 1; i++)
      {
        ...
        std::move(raw_pointer_cast(base::data_ + base::pointers_begin_[i + 1]),
                  raw_pointer_cast(base::data_ + base::pointers_end_[i + 1]),
                  raw_pointer_cast(base::data_ + base::pointers_end_[i]));
        std::move(raw_pointer_cast(base::jdata_ + base::pointers_begin_[i + 1]),
                  raw_pointer_cast(base::jdata_ + base::pointers_end_[i + 1]),
                  raw_pointer_cast(base::jdata_ + base::pointers_end_[i]));
...
protected:
  struct row_reference
  {
    ...
    this_t& self_;
    IndxType i_;
    using value_type   = typename this_t::value_type;  
    using element      = value_type;
    using element_type = value_type;
    using element_ptr  = typename this_t::element_ptr; 
    struct element_reference
    {
      row_reference& self_;
      IndxType j_;
  ...
  struct const_row_reference
  {
    ...
    const this_t& self_;
    const IndxType i_;
    using value_type   = typename this_t::value_type const;
    using element      = value_type;
    using element_type = value_type;
    using element_ptr  = typename this_t::element_ptr const;
    ...
    auto shape() const { return std::array<size_type, 1>{{self_.size2_}}; }
    template<typename integer_type = size_type>
    auto size([[maybe_unused]] integer_type d) const
    {
      return size_type{self_.size2_};
    }
  };
...
public:
  ...
  {
    RUNTIME_CHECK(arr[0] >= 0 && size_type(arr[1]) <= base::size1_, "");
    RUNTIME_CHECK(arr[0] < arr[1], "");
  ...
  using const_sub_matrix = csr_matrix_ref<const_ValTypePtr, const_IndxTypePtr, const_IntTypePtr>;
  template<class integer_type>
  const_sub_matrix operator[](std::array<integer_type, 2> const& arr) const
  {
    RUNTIME_CHECK(arr[0] >= 0 && size_type(arr[1]) <= base::size1_, "");
    RUNTIME_CHECK(arr[0] < arr[1], "");
    size_type disp = static_cast<size_type>(base::pointers_begin_[arr[0]] - base::pointers_begin_[0]);
    ...
    for (size_type i = arr[0]; i < arr[1]; i++)
      cap += base::capacity(i);
    // careful!!! elements or row 'r' are always indexed with pointer_begin[r]-pointer_begin[0]
    return const_sub_matrix(tp_ul_ul{(arr[1] - arr[0]), base::size2_}, tp_ul_ul{arr[0], 0},
                      tp_ul_ul{base::global_origin1_ + arr[0], base::global_origin2_}, cap, base::data_ + disp,
                      base::jdata_ + disp, base::pointers_begin_ + static_cast<size_type>(arr[0]),
                      base::pointers_end_ + static_cast<size_type>(arr[0]));
  }
  ...
  {
    // limited right now
    RUNTIME_CHECK(base::capacity_ > 0, "");
    RUNTIME_CHECK(base::size1_ > 0, "");
    RUNTIME_CHECK(base::size2_ > 0, "");
    RUNTIME_CHECK(arr[0] >= 0 && size_type(arr[1]) <= base::size1_, "");
    RUNTIME_CHECK(arr[2] >= 0 && size_type(arr[3]) <= base::size2_, "");
    RUNTIME_CHECK(arr[0] < arr[1], "");
    RUNTIME_CHECK(arr[2] < arr[3], "");
    ...
    else
    {
      ...
      auto ref_col =
          std::lower_bound(raw_pointer_cast(base::jdata_ + base::pointers_begin_[arr[0]] - p0),
                           raw_pointer_cast(base::jdata_ + base::pointers_end_[arr[0]] - p0), static_cast<IndxType>(arr[2]));
      for (std::size_t r = arr[0]; r < arr[1]; r++)
      {
        auto br =
            std::lower_bound(raw_pointer_cast(base::jdata_ + base::pointers_begin_[r] - p0),
                             raw_pointer_cast(base::jdata_ + base::pointers_end_[r] - p0), static_cast<IndxType>(arr[2]));
        auto er =
            std::lower_bound(raw_pointer_cast(base::jdata_ + base::pointers_begin_[r] - p0),
                             raw_pointer_cast(base::jdata_ + base::pointers_end_[r] - p0), static_cast<IndxType>(arr[3]));
      ...
      ptrb.emplace_back(ptre.back());
      auto d0 = std::distance(raw_pointer_cast(base::jdata_), ref_col);
