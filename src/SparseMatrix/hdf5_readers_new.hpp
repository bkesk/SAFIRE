...
#ifndef SFQMC_AFQMC_HDF5_READERS_NEW_HPP
#define SFQMC_AFQMC_HDF5_READERS_NEW_HPP
...
inline bool multiple_reader_hdf5_SpMat(SpMatrix& SpM,
                                       ...
                                       int n_working_cores)
{
  using VType = typename SpMatrix::element_type;
  ...
  if (coreid < n_working_cores)
  {
    ...
    for (int iter = 0; iter < niter; iter++)
    {
      ...
      if (myblock_number < last_local_block)
      {
        ...
        {
          app_error(" Error in multiple_reader_hdf5_SpMat: Problems reading ***_index_{} dataset.", myblock_number);
          return false;
        }
        if (!dump.readEntry(vvec, std::string("vals_") + std::to_string(myblock_number)))
        {
          app_error(" Error in multiple_reader_hdf5_SpMat: Problems reading ***_vals_{} datset.", myblock_number);
          return false;
        }
      }
      for (int k = first_block, ipr = 0; k < last_block; k++, ipr++)
      {
        ...
        for (int ik = 0, ikend = block_size[k]; ik < ikend; ik++)
          ...
          {
            buff.emplace_back(std::make_tuple(ivec2[2 * ik], ivec2[2 * ik + 1], vvec2[ik]));
            if (buff.size() == nmax)
            {
...
template<class SpMatrix, class SpMatrix_Partition, class task_group>
inline bool single_reader_hdf5_SpMat([[maybe_unused]] SpMatrix& SpM,
                                     [[maybe_unused]] SpMatrix_Partition& split,
                                     [[maybe_unused]] hdf_archive& dump,
                                     [[maybe_unused]] int nblk,
                                     [[maybe_unused]] task_group& TG)
{
  return true;
}


template<typename VType, class SpMatrix_Partition, class IVec, class task_group>
                                          ...
                                          int n_working_cores)
{
  ...
  if (coreid < n_working_cores)
  {
    ...
    for (int ib = first_local_block; ib < last_local_block; ib++)
    {
      ...
      {
        app_error(" Error in multiple_reader_count_entries: Problems reading ***_index_{} dataset. ", ib);
        return false;
      }
      if (!dump.readEntry(vvec, std::string("vals_") + std::to_string(ib)))
      {
        app_error(" Error in multiple_reader_count_entries: Problems reading ***_vals_{} dataset. ", ib);
        return false;
      }
      if (byRow)
      {
                                        ...
                                        task_group& TG)
{
  ...
  for (int ib = 0; ib < nblk; ib++)
  {
    ...
    if (!dump.readEntry(ivec, std::string("index_") + std::to_string(ib)))
    {
      app_error(" Error in single_reader_count_entries: Problems reading ***_index_{} dataset. ", ib);
      return false;
    }
    if (!dump.readEntry(vvec, std::string("vals_") + std::to_string(ib)))
    {
      app_error(" Error in single_reader_count_entries: Problems reading ***_vals_{} dataset. ", ib);
      return false;
    }
    if (byRow)
    {
...
{
  RUNTIME_CHECK(n_working_cores >= 1, "");
  if (parallel_read)
                                     ...
                                     int n_working_cores = 1)
{
  RUNTIME_CHECK(n_working_cores >= 1, "");
  if (parallel_read)
    return multiple_reader_count_entries<VType>(dump, split, nblk, byRow, counts, TG, n_working_cores);
  else
  ...
  if(core_rank==0 && node_number==0) {
    ...
    for(int i=0; i<nb_loc; i++) {
      ...
      for(int k=0; k<nterms; k++, cnt++, row++, col++, val++) {
        ivec.push_back(*row);
        ivec.push_back(*col);
        vvec.push_back(static_cast<VType>(*val));  
      }
    ...
    for(int rk=1; rk<nnodes_per_TG; rk++) {
      ...
      do { 
        ...
        if(st.MPI_TAG == MORE) {
          RUNTIME_CHECK(nterms == nterms_per_blk, "");
