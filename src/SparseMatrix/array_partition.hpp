...
#ifndef SFQMC_AFQMC_ARRAY_PARTITION_HPP
#define SFQMC_AFQMC_ARRAY_PARTITION_HPP
...
public:
      ...
      : byrow_(row_), ngr(ngr_), ngc(ngc_), r0(r0_), r1(r1_), c0(c0_), c1(c1_), cut(std::abs(cut_))
  {
    RUNTIME_CHECK(ngr > 0, "");
    RUNTIME_CHECK(ngc > 0, "");
    //RUNTIME_CHECK(r0 >= 0, ""); // this will always pass. r0 is unsigned.
    //RUNTIME_CHECK(c0 >= 0, ""); // this will always pass. c0 is unsigned.
    RUNTIME_CHECK(r1 > r0, "");
    RUNTIME_CHECK(c1 > c0, "");
    RUNTIME_CHECK(r1 <= ngr, "");
    RUNTIME_CHECK(c1 <= ngc, "");
  }
...
struct simple_matrix_partition
{
  ...
  inline void partition(const task_group& TG, bool byRow, const std::vector<IType>& counts, std::vector<IType>& sets)
  {
    ...
    IType cnt  = 0;
    utils::check(nblk >= nnodes);
    if (byRow)
      utils::check(nblk == nrows);
    else
      utils::check(nblk == ncols);
    sets.resize(nnodes + 1);
    sets[0] = 0;
    if (nnodes > 1)
    {
                                 ...
                                 std::vector<IType>& sets)
  {
    ...
    int nblk  = counts.size();
    IType cnt = 0;
    RUNTIME_CHECK(nblk >= ngrps, "");
    if (byRow)
      RUNTIME_CHECK(nblk == nrows, "");
    else
      RUNTIME_CHECK(nblk == ncols, "");
    sets.resize(ngrps + 1);
    sets[0] = 0;
    if (ngrps > 1)
    {
                            ...
                            std::vector<IType>& sets)
  {
    ...
    RUNTIME_CHECK(nblk >= ncores, "");
    if (byRow)
      RUNTIME_CHECK(nblk == r1 - r0, "");
    else
      RUNTIME_CHECK(nblk == c1 - c0, "");
    sets.resize(ncores + 1);
    sets[0] = 0;
    if (ncores > 1)
    {
