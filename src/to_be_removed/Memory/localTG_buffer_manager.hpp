...
public:
  ...
  LocalTGBufferManager(boost::mpi3::shared_communicator& local, size_t size) : DeviceBufferManager(size, true) {}
...
{
public:
  using generator_t = BufferAllocatorGenerator<shm_memory_resource, shm_constructor<std::byte>>;

  template<class T>
  using allocator_t = typename generator_t::template allocator<T>;

  LocalTGBufferManager(boost::mpi3::shared_communicator& local, size_t size)
  {
    require(false);
    generator = std::make_unique<generator_t>(
		::shm::memory_resource_shm_ptr_with_raw_ptr_dispatch{std::addressof(local)}, size);//,
//                shm_constructor<std::byte>{local});
  }
