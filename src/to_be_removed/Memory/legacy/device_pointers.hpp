...
// MAM: this should be a call to alloc_alloc_uninitialized_copy
...
device_pointer<Q> copy_n_cast(T* const A, Size n, device_pointer<Q> B)
{
  ...
  device_allocator<decay_T> alloc{};
  device_pointer<decay_T> p(alloc.allocate(n));
  copy_n(A, n, p);  
  copy_n_cast(p, n, B);
...
template<class Alloc, typename T, typename Q>
device_pointer<Q> uninitialized_copy(Alloc& a, T* Abeg, T* Aend, device_pointer<Q> B)
{
  static_assert( std::is_trivially_assignable<Q&, T>{} ,"!");
  return copy_n(Abeg, std::distance(Abeg, Aend), B);
}
...
template<class Alloc, typename T, typename Q>
Q* uninitialized_copy(Alloc& a, device_pointer<T> const Abeg, device_pointer<T> const Aend, Q* B)
{
  static_assert( std::is_trivially_assignable<Q&, T>{} ,"!");
  return copy_n(Abeg, std::distance(Abeg, Aend), B);
}
...
template<class Alloc, typename T, typename Q>
Q* alloc_uninitialized_copy(Alloc& a, device_pointer<T> const Abeg, device_pointer<T> const Aend, Q* B)
{
  static_assert( std::is_trivially_assignable<Q&, T>{} ,"!");
  return copy_n(Abeg, std::distance(Abeg, Aend), B);
}
...
template<typename T, typename Q>
void fill2D(int n, int m, legacy::device::device_pointer<T> first, int lda, Q val)
{
  RUNTIME_CHECK(lda >= m, "");
...
template<typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> fill_n(
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n,
    T const& val)
{
  if (n == 0)
...
template<typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> fill(
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last,
    T const& val)
{
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_fill_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n,
    T const& val)
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_fill(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last,
    T const& val)
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_fill_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n,
    T const& val)
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_fill(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last,
    T const& val)
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> copy(
    multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> first,
    multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> last,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
{
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> copy(ForwardIt first,
                                                            ForwardIt last,
                                                            multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
{
  RUNTIME_CHECK(stride(first) == stride(last), "");
...
ForwardIt copy(multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> first,
               multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> last,
               ForwardIt dest)
{
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> copy_n(
    multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> first,
    Size N,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
{
...
ForwardIt copy_n(multi::array_iterator<Q1, 1, legacy::device::device_pointer<Q2>> first, Size N, ForwardIt dest)
{
  using T = typename std::decay<typename ForwardIt::value_type>::type;
  static_assert(std::is_same<typename std::decay<Q1>::type, T>::value, "Wrong dispatch.");
  static_assert(std::is_same<typename std::decay<Q2>::type, T>::value, "Wrong dispatch.");
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_copy(
                                                            T* first,
                                                            T* last,
                                                            multi::array_iterator<Q, 1, legacy::device::device_pointer<QQ>> dest)
{
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_copy(
    multi::array_iterator<Q, 1, legacy::device::device_pointer<QQ>> first,
    multi::array_iterator<Q, 1, legacy::device::device_pointer<QQ>> last,
    multi::array_iterator<T, 1, legacy::device::device_pointer<TT>> dest)
{
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_copy(
    Alloc& a,
    ForwardIt first,
    ForwardIt last,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_copy( 
                         Alloc &a,
                         multi::array_iterator<Q, 1, legacy::device::device_pointer<Q>> first,
                         multi::array_iterator<Q, 1, legacy::device::device_pointer<Q>> last,
                         multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest ){
  static_assert(std::is_same<typename std::decay<Q>::type,T>::value,"Wrong dispatch.");
  RUNTIME_CHECK(stride(first) == stride(last), "");
...
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_copy_n( 
                           Alloc &a,
                           multi::array_iterator<Q, 1, legacy::device::device_pointer<Q>> first,
                           Size N,
                           multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest ){
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_default_construct_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n)
{
...
template<class Alloc, typename T>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_default_construct(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last)
{
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_value_construct_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n)
{
...
template<class Alloc, typename T>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> uninitialized_value_construct(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last)
{
...
template<class Alloc, class T, class ForwardIt, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_copy_n(
    Alloc& a,
    ForwardIt first,
    Size n,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
...
template<class Alloc, class T, class ForwardIt, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_move_n(
    Alloc& a,
    ForwardIt first,
    Size n,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> dest)
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_default_construct_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n)
{
...
template<class Alloc, typename T>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_default_construct(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last)
{
...
template<class Alloc, typename T, typename Size>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_value_construct_n(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    Size n)
{
...
template<class Alloc, typename T>
multi::array_iterator<T, 1, legacy::device::device_pointer<T>> alloc_uninitialized_value_construct(
    Alloc& a,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> first,
    multi::array_iterator<T, 1, legacy::device::device_pointer<T>> last)
{
