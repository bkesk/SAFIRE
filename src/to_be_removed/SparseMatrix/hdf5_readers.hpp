...
#ifndef SFQMC_AFQMC_HDF5_READERS_HPP
#define SFQMC_AFQMC_HDF5_READERS_HPP
...
inline bool multiple_reader_hdf5_SpMat(SpMatrix& SpM,
                                       ...
                                       int n_working_cores)
{
  using value_type = typename std::decay_t<SpMatrix>::element_type;
  ...
  if (coreid < n_working_cores)
  {
    ...
    for (int iter = 0; iter < niter; iter++)
    {
      ...
      if (myblock_number < last_local_block)
      {
        ...
        {
          app_error(" Error in multiple_reader_hdf5_SpMat: Problems reading ***_vals_{} dataset. ", myblock_number);
          return false;
        }
      }
      for (int k = first_block, ipr = 0; k < last_block; k++, ipr++)
      {
        ...
        for (int ik = 0, ikend = block_size[k]; ik < ikend; ik++)
          if (split.local(ivec2[2 * ik], ivec2[2 * ik + 1], vvec2[ik]))
            SpM.add(ivec2[2 * ik], ivec2[2 * ik + 1], static_cast<value_type>(vvec2[ik]), need_lock);
      }
    }
  }
...
template<class value_type, class SpMatrix_Partition, class IVec, class task_group>
...
template<typename value_type, class SpMatrix_Partition, class IVec, class task_group>
...
template<class SpMatrix, class task_group>
inline std::tuple<int, int> write_hdf5_SpMat(SpMatrix& SpM,
                                             hdf_archive& dump,
                                             std::string name,
                                             int nterms_per_blk,
                                             task_group& TG)
  ...
  if (core_rank == 0 && node_number == 0)
  {
    ...
    for (int i = 0; i < nb_loc; i++)
    {
      ...
      for (int k = 0; k < nterms; k++, cnt++, row++, col++, val++)
      {
        ivec.push_back(*row);
        ivec.push_back(*col);
        vvec.push_back(static_cast<value_type>(*val));
      }
