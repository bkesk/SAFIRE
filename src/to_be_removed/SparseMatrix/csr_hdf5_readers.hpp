...
#ifndef SFQMC_AFQMC_CSR_HDF5_READERS_HPP
#define SFQMC_AFQMC_CSR_HDF5_READERS_HPP
...
inline SparseArray2D HDF2CSR(hdf_archive& dump, shared_communicator& node) //, Alloc alloc_)
{
  ...
  if (node.root())
  {
    ...
    RUNTIME_CHECK(ptrb.size() == nrows, "");
    RUNTIME_CHECK(ptre.size() == nrows, "");
...
inline void CSR2HDF(hdf_archive& dump, SparseArray2D const& SpM)
{
  ...
  }
  {
    ...
    for (size_type i = 0; i < nrows; i++)
    {
      ...
      std::copy_n(raw_pointer_cast(SpM.non_zero_values_data(i)), nt, data.data() + cnt);
      cnt += nt;
    }
  ...
  }
  {
    ...
    {
      size_type nt = SpM.num_non_zero_elements(i);
      std::copy_n(raw_pointer_cast(SpM.non_zero_indices2_data(i)), nt, jdata.data() + cnt);
      cnt += nt;
    }
                                     ...
                                     int n_working_cores)
{
  ...
  if (coreid < n_working_cores)
  {
    ...
    std::vector<value_type> vvec, vvec2;
    ...
    for (int iter = 0; iter < niter; iter++)
    {
      ...
      if (myblock_number < last_local_block)
      {
        ...
        if (block_size[myblock_number] > 0)
        {
          ...
          {
            app_error(" Error in multiple_reader_hdf5_csr: Problems reading index_{} dataset.", myblock_number);
          ...
          {
            app_error(" Error in multiple_reader_hdf5_csr: Problems reading valsx_{} dataset.", myblock_number);
      ...
      for (int k = first_block, ipr = 0; k < last_block; k++, ipr++)
      {
        ...
        if (ipr == nodeid)
        {
          RUNTIME_CHECK(myblock_number == k, "");
                                                                ...
                                                                task_group& TG)
{
  ...
  int nblk = block_size.size();
  auto [first_block, last_block] = FairDivideBoundary(rank, nblk, size);
  ...
  for (int k = first_block, ipr = 0; k < last_block; k++, ipr++)
  {
    ...
    if (block_size[k] > 0)
    {
      ...
      {
        app_error(" Error in multiple_reader_hdf5_csr: Problems reading index_{} dataset. ", k);;
        APP_ABORT(" Error in multiple_reader_hdf5_csr: Problems reading index_ dataset. ");
      }
      if (!readComplexOrReal(dump, std::string("vals_") + std::to_string(k), vvec))
      {
        app_error(" Error in multiple_reader_hdf5_csr: Problems reading vals_{} dataset. ", k);;
        APP_ABORT(" Error in multiple_reader_hdf5_csr: Problems reading vals_ dataset. ");
      }
      for (int ik = 0, ikend = block_size[k]; ik < ikend; ik++)
      {
 ...
 *       on a node. 
 */
template<class value_type, class matrix_partition, class IVec, class task_group>
inline void multiple_reader_local_count(hdf_archive& dump,
                                        ...
                                        int n_working_cores)
{
  RUNTIME_CHECK(counts.size() == split.range(), "");
  std::fill(counts.begin(), counts.end(), 0);
  RUNTIME_CHECK(sizeof(counts[0]) == sizeof(int), "");
    ...
    for (int iter = 0; iter < niter; iter++)
    {
      ...
      if (myblock_number < last_local_block)
      {
        ...
        if (block_size[myblock_number] > 0)
        {
          if (!dump.readEntry(ivec, std::string("index_") + std::to_string(myblock_number)))
          {
            app_error(" Error in multiple_reader_hdf5_SpMat: Problems reading index_{} dataset. ", myblock_number);
            APP_ABORT(" Error in multiple_reader_hdf5_SpMat: Problems reading index dataset. ");
          }
          if (!readComplexOrReal(dump, std::string("vals_") + std::to_string(myblock_number), vvec))
          {
            app_error(" Error in multiple_reader_hdf5_SpMat: Problems reading vals_{} dataset. ", myblock_number);
  ...
  if (coreid < n_working_cores)
  {
    ...
    for (int ib = first_local_block; ib < last_local_block; ib++)
    {
      ...
      if (block_size[ib] > 0)
      {
        ...
        {
          app_error(" Error in multiple_reader_count_entries: Problems reading index_{} dataset. ", ib);
        ...
        {
          app_error(" Error in multiple_reader_count_entries: Problems reading vals_{} dataset. ", ib);
                                                     ...
                                                     int nread = -1)
{
  ...
  if (distribute_Ham)
  {
    ...
    if (TG.Global().root())
    {
      ...
      app_log(3, " Partitioning of columns in column_distributed_CSR_from_HDF: ");
      for (int i = 0; i <= TG.getNGroupsPerTG(); i++)
        app_log(3, "{} ",sets[i]);
      app_log(3, "");
      app_log(3, " Number of terms in each partitioning: ");
      for (int i = 0; i < TG.getNGroupsPerTG(); i++)
        app_log(3, "{} ",accumulate(col_counts.begin() + sets[i], col_counts.begin() + sets[i + 1], 0));
      app_log(3, "");
    }
...
inline void write_distributed_CSR_to_HDF(SparseArray2D const& SpM, hdf_archive& dump, task_group_& TG)
{
  ...
  else
  {
    if (TG.Global().root())
    {
      ...
      RUNTIME_CHECK(cnt == ((nnz - size_t(1)) / CSR_HDF_BLOCK_SIZE + 1), "");
      dump.write(block_sizes, "block_sizes");
    }
  }
  TG.Global().barrier();
