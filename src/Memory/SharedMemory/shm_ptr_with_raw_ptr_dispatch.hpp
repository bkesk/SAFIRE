...
#ifndef SHM_PTR_WITH_RAW_PTR_DISPATCH_HPP
#define SHM_PTR_WITH_RAW_PTR_DISPATCH_HPP
...
{
  using T = const void;
  using pointer = shm_ptr_with_raw_ptr_dispatch;
...
struct shm_ptr_with_raw_ptr_dispatch
{
  ...
  explicit operator void *() const
  {
    if (wSP_ == nullptr)
      ...
      return reinterpret_cast<void*>(wSP_->base(0) + offset);
  }
  ...
  { 
    if (wSP_ == nullptr)
      ...
      return reinterpret_cast<const void*>(wSP_->base(0) + offset);
  }	
...
*/
  ...
  shm_ptr_with_raw_ptr_dispatch operator--(int)
  {
    shm_ptr_with_raw_ptr_dispatch t(*this);
    offset -= sizeof(T);
    return t;
  }
  shm_ptr_with_raw_ptr_dispatch operator++(int)
  ...
  static element_type* raw_pointer_cast(shm_ptr_with_raw_ptr_dispatch p) noexcept
  {
    return reinterpret_cast<element_type*>(p.wSP_->base(0) + p.offset);
  }
  friend pointer raw_pointer_cast(shm_ptr_with_raw_ptr_dispatch const& p)
  {
    return shm_ptr_with_raw_ptr_dispatch::raw_pointer_cast(p);
  }
...
struct allocator_shm_ptr_with_raw_ptr_dispatch
{
  ...
  using void_pointer = shm_ptr_with_raw_ptr_dispatch<void>;
  using const_void_pointer = shm_ptr_with_raw_ptr_dispatch<const void>;
  using size_type       = mpi3::size_t;                  // std::size_t;
  using difference_type = std::make_signed_t<size_type>; //std::ptrdiff_t;
  template<class T2> using rebind = allocator_shm_ptr_with_raw_ptr_dispatch<T2>;

  mpi3::shared_communicator* comm_;
  allocator_shm_ptr_with_raw_ptr_dispatch() = delete;
  allocator_shm_ptr_with_raw_ptr_dispatch(mpi3::shared_communicator& comm) : comm_(std::addressof(comm)) {}
  ...
  allocator_shm_ptr_with_raw_ptr_dispatch select_on_container_copy_construction() const{return *this;}

  shm_ptr_with_raw_ptr_dispatch<T> allocate(size_type n, const_void_pointer = nullptr)
  {
    ...
    // MAM: I think it is a good idea to zero out memory
    if(comm_->root()) memset(ret.wSP_->base(0), 0, n); 
    comm_->barrier();
    return ret;
  }
  ...
  {
    utils::check((*this) == other); // TODO make comm a shared_ptr
    return *this;
  }
  ...
  // this routine synchronizes
  template<class U, class... As>
  void destroy(U p, [[maybe_unused]] As&&... as)
  {
    destroy_n(p, 1);
  }
};
...
template<class T>
struct constructor_shm_ptr_with_raw_ptr_dispatch
{
  template<class T2> using rebind = constructor_shm_ptr_with_raw_ptr_dispatch<T2>;
  using value_type         = T;
  using pointer         = shm_ptr_with_raw_ptr_dispatch<T>;
  using const_pointer   = shm_ptr_with_raw_ptr_dispatch<T const>;
  using void_pointer = shm_ptr_with_raw_ptr_dispatch<void>;
  ...
  constructor_shm_ptr_with_raw_ptr_dispatch()                         = default;
  ~constructor_shm_ptr_with_raw_ptr_dispatch()                        = default;
  constructor_shm_ptr_with_raw_ptr_dispatch(constructor_shm_ptr_with_raw_ptr_dispatch const& other) = default;
  template<class U>
  constructor_shm_ptr_with_raw_ptr_dispatch([[maybe_unused]] constructor_shm_ptr_with_raw_ptr_dispatch<U> const& other)
  {}
...
struct memory_resource_shm_ptr_with_raw_ptr_dispatch
{
  ...
  mpi3::shared_communicator* comm_ = nullptr;
...
shm_ptr_with_raw_ptr_dispatch<T> fill_n(shm_ptr_with_raw_ptr_dispatch<T> first, Size n, TT const& val)
{
  ...
  if (first.wSP_->get_group().root())
    std::fill_n(raw_pointer_cast(first), n, val);
  first.wSP_->fence();
  first.wSP_->fence();
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_fill_n(shm_ptr_with_raw_ptr_dispatch<T> first, Size n, TT const& val)
{
  ...
  first.wSP_->fence();
  if (first.wSP_->get_group().root())
    std::uninitialized_fill_n(raw_pointer_cast(first), n, val);
  first.wSP_->fence();
  first.wSP_->fence();
  mpi3::communicator(first.wSP_->get_group(), 0).barrier();
  return first + n;
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_fill_n([[maybe_unused]] Alloc& a,
                                                      ...
                                                      TT const& val)
{
  ...
  first.wSP_->fence();
  if (first.wSP_->get_group().root())
    std::uninitialized_fill_n(raw_pointer_cast(first), n, val); // change to to_pointer
  first.wSP_->fence();
  first.wSP_->fence();
  mpi3::communicator(first.wSP_->get_group(), 0).barrier();
  return first + n;
...
template<class Alloc, typename T, typename Size, typename TT>
shm_ptr_with_raw_ptr_dispatch<T> alloc_uninitialized_fill_n([[maybe_unused]]  Alloc& a,
                                                            shm_ptr_with_raw_ptr_dispatch<T> first,
                                                            Size n,
                                                            TT const& val)
{
...
shm_ptr_with_raw_ptr_dispatch<T> destroy_n(shm_ptr_with_raw_ptr_dispatch<T> first, Size n)
{
  ...
  if (first.wSP_->get_group().root())
  {
    auto first_ptr = raw_pointer_cast(first);
...
shm_ptr_with_raw_ptr_dispatch<T> destroy_n([[maybe_unused]] Alloc& a, shm_ptr_with_raw_ptr_dispatch<T> first, Size n)
{
  if (n == 0)
    return first;
  first.wSP_->fence();
...
template<class Alloc, typename T, typename Size>
shm_ptr_with_raw_ptr_dispatch<T> alloc_destroy_n([[maybe_unused]] Alloc& a, shm_ptr_with_raw_ptr_dispatch<T> first, Size n)
{
  if (n == 0)
    return first;
  first.wSP_->fence();
...
shm_ptr_with_raw_ptr_dispatch<T> copy_n_cast(It1 first, Size n, shm_ptr_with_raw_ptr_dispatch<T> d_first)
{
  ...
  if (d_first.wSP_->get_group().root())
    copy_n_cast(raw_pointer_cast(first), n, raw_pointer_cast(d_first));
  d_first.wSP_->fence();
...
shm_ptr_with_raw_ptr_dispatch<T> copy_n(It1 first, Size n, shm_ptr_with_raw_ptr_dispatch<T> d_first)
{
  ...
  using std::copy_n;
  if (d_first.wSP_->get_group().root())
    copy_n(first, n, raw_pointer_cast(d_first));
  d_first.wSP_->fence();
  mpi3::communicator(d_first.wSP_->get_group(), 0).barrier();
  return d_first + n;
}
...
shm_ptr_with_raw_ptr_dispatch<T> copy(It1 first, It1 last, shm_ptr_with_raw_ptr_dispatch<T> d_first)
{
  ...
  using std::copy;
  if (d_first.wSP_->get_group().root())
    copy(first, last, raw_pointer_cast(d_first));
  first.wSP_->fence();
  mpi3::communicator(d_first.wSP_->get_group(), 0).barrier();
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_copy_n(It1 f, Size n, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  ...
  if (d.wSP_->get_group().root())
    uninitialized_copy_n(f, n, raw_pointer_cast(d));
  d.wSP_->fence();
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_copy_n([[maybe_unused]] Alloc& a, It1 f, Size n, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  if (n == 0)
    return d;
  d.wSP_->fence();
...
template<class Alloc, class It1, class Size, typename T>
shm_ptr_with_raw_ptr_dispatch<T> alloc_uninitialized_copy_n([[maybe_unused]] Alloc& a, It1 f, Size n, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  if (n == 0)
    return d;
  d.wSP_->fence();
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_copy(It1 f, It1 l, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  ...
  if (d.wSP_->get_group().root())
    uninitialized_copy(f, l, raw_pointer_cast(d));
  d.wSP_->fence();
  mpi3::communicator(d.wSP_->get_group(), 0).barrier();
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_copy([[maybe_unused]] Alloc& a, It1 f, It1 l, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  using std::distance;
  if (distance(f, l) == 0)
    return d;
...
template<class Alloc, class It1, typename T>
shm_ptr_with_raw_ptr_dispatch<T> alloc_uninitialized_copy([[maybe_unused]] Alloc& a, It1 f, It1 l, shm_ptr_with_raw_ptr_dispatch<T> d)
{
  using std::distance;
  if (distance(f, l) == 0)
    return d;
...
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_default_construct_n([[maybe_unused]] Alloc& a, shm_ptr_with_raw_ptr_dispatch<T> f, Size n)
{
  return uninitialized_default_construct_n(f, n);
}

template<class Alloc, class T, class Size>
shm_ptr_with_raw_ptr_dispatch<T> uninitialized_value_construct_n([[maybe_unused]] Alloc& a, shm_ptr_with_raw_ptr_dispatch<T> f, Size n)
{
  return uninitialized_value_construct_n(f, n);
}

template<class Alloc, class T, class Size>
shm_ptr_with_raw_ptr_dispatch<T> alloc_uninitialized_default_construct_n([[maybe_unused]] Alloc& a,
...
template<class Alloc, class T, class Size>
shm_ptr_with_raw_ptr_dispatch<T> alloc_uninitialized_value_construct_n([[maybe_unused]]  Alloc& a,
                                                                       shm_ptr_with_raw_ptr_dispatch<T> f,
                                                                       Size n)
{
  return uninitialized_value_construct_n(f, n);
...
multi::array_iterator<T, 1, shm::shm_ptr_with_raw_ptr_dispatch<T>> fill_n(
    ...
    Q const& val)
{
  RUNTIME_CHECK(stride(first) == stride(last), "");
  return fill_n(first, std::distance(first, last), val);
}
    ...
    Q const& val)
{
  ...
  if (first.wSP_->get_group().root())
  {
    ...
    {
      for (; n > 0; ++current, --n)
        (::new ((void*)raw_pointer_cast(base(current))) T(val));
    }
    catch (...)
    {
      throw;
...
//  static_assert(std::is_same<typename std::decay<Q>::type, T>::value, "Wrong dispatch.");
  ...
  if (base(dest).wSP_->get_group().root())
  {
    ...
    {
      for (; n > 0; ++f, ++d, --n)
        (::new ((void*)raw_pointer_cast(base(d))) T(*f));
    }
    catch (...)
    {
      throw;
  ...
  base(first).wSP_->fence();
  {
    ...
    try
    {
      for (; first != last; ++first, ++d)
        (::new ((void*)raw_pointer_cast(base(d))) T(*first));
    }
    catch (...)
    {
      throw;
    ...
    Size n)
{
  ...
  if (base(f).wSP_->get_group().root())
  {
    ...
    {
      for (; n > 0; ++current, --n)
        (::new ((void*)raw_pointer_cast(base(current))) T());
      return current;
    }
    catch (...)
    {
