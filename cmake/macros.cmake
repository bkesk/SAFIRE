...
#   RUN_AF_APP
#     Run SAFIRE with a given number of threads and MPI processes
#
#   SF_RUN_AND_CHECK
#     Run SAFIRE and check scalar output values.  This is the
#     primary function used for system tests.
#
#   SIMPLE_RUN_AND_CHECK
#     Run SAFIRE on the given input file and check output
...
FUNCTION( RUN_AF_APP_NO_COPY TESTNAME WORKDIR PROCS THREADS TEST_ADDED TEST_LABELS ${ARGN} )
    MATH( EXPR TOT_PROCS "${PROCS} * ${THREADS}" )
    SET( QMC_APP "${AF_BINARY_DIR}/bin/af_app" )
...
# Runs SAFIRE
#  Note that TEST_ADDED is an output variable
FUNCTION( RUN_AF_APP TESTNAME SRC_DIR PROCS THREADS TEST_ADDED TEST_LABELS ${ARGN} )
    COPY_DIRECTORY_MAYBE_USING_SYMLINK( "${SRC_DIR}" "${CMAKE_CURRENT_BINARY_DIR}/${TESTNAME}" )
    SET( TEST_ADDED_TEMP FALSE )
    SET( TEST_LABELS_TEMP "" )
    RUN_AF_APP_NO_COPY( ${TESTNAME} ${CMAKE_CURRENT_BINARY_DIR}/${TESTNAME} ${PROCS} ${THREADS} TEST_ADDED_TEMP TEST_LABELS_TEMP ${ARGN} )
    SET( ${TEST_ADDED} ${TEST_ADDED_TEMP} PARENT_SCOPE )
    SET( ${TEST_LABELS} ${TEST_LABELS_TEMP} PARENT_SCOPE )
ENDFUNCTION()
...
# INPUT_FILE - input file to Auxilary Fields
...
FUNCTION(QMC_RUN_AND_CHECK BASE_NAME BASE_DIR PREFIX INPUT_FILE PROCS THREADS SHOULD_SUCCEED)
    ...
    RUN_AF_APP(${FULL_NAME} ${BASE_DIR} ${PROCS} ${THREADS} TEST_ADDED TEST_LABELS ${INPUT_FILE})
    IF ( TEST_ADDED )
        SET_PROPERTY(TEST ${FULL_NAME} APPEND PROPERTY LABELS "SFQMC")
    ENDIF()
            ...
            IF(MOD_IDX0 EQUAL 1)
                ...
                FOREACH(SCALAR_CHECK IN LISTS SCALAR_CHECK_TYPE)
                    ...
                    IF (IDX1 GREATER -1)
                        ...
                        SET_PROPERTY( TEST ${TEST_NAME} APPEND PROPERTY LABELS "SFQMC-checking-results" )
...
# BASE_DIR - source location of test input files
# PREFIX - prefix for output files
# INPUT_FILE - input file to SAFIRE
...
FUNCTION( COVERAGE_RUN TESTNAME SRC_DIR PROCS THREADS ${ARGN} )
    ...
    SET( TEST_ADDED FALSE )
    SET( TEST_LABELS "" )
    RUN_AF_APP( ${FULLNAME} ${SRC_DIR} ${PROCS} ${THREADS} TEST_ADDED TEST_LABELS ${ARGN} )
    IF (TEST_ADDED)
