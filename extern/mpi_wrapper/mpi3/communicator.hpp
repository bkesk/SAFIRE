...
//	}
 ...
 private:
	...
	auto all_to_all_n(
		...
		using count_type_ = int;
		MPI_(Alltoall)(
			detail::data(  first), static_cast<count_type_>(count/* / sz*/), detail::basic_datatype<typename std::iterator_traits<It1>::value_type>{},
			detail::data(d_first), static_cast<count_type_>(count/* / sz*/), detail::basic_datatype<typename std::iterator_traits<It2>::value_type>{},
			impl_
		);
		return d_first + count;
	}

	template<class It1, class It2, typename Itsz>
	void all_to_all_v_n(
		It1 first,
		/**/ detail::contiguous_iterator_tag /*tag*/,
		/**/ detail::basic_tag /*tag*/,
		...
		MPI_(Alltoallv)(
			detail::data(  first), count1, disp1, detail::basic_datatype<typename std::iterator_traits<It1>::value_type>{},
			detail::data(d_first), count2, disp2, detail::basic_datatype<typename std::iterator_traits<It2>::value_type>{},
			impl_
		);
		//return d_first + (*(count2+rank()))*size();
	}
...
private:
 ...
 public:
	...
	auto all_to_all_inplace_n(It1 first, Size count) {
		...
		using count_type_ = int;
		MPI_(Alltoall)(
			in_place           , 0*static_cast<count_type_>(count)/*/sz*/, detail::basic_datatype<typename std::iterator_traits<It1>::value_type>{},
			detail::data(first),   static_cast<count_type_>(count)/*/sz*/, detail::basic_datatype<typename std::iterator_traits<It1>::value_type>{},
			impl_
		);
		return first + count;
...
//	{ TODO implement }
 ...
 public:
	...
	auto all_reduce_n(
		...
		using count_type_ = int;
		MPI_(Allreduce)(
			detail::data(first), detail::data(d_first), static_cast<count_type_>(count), detail::basic_datatype<typename std::iterator_traits<It1>::value_type>{},  // TODO(correaa) use safe cast
