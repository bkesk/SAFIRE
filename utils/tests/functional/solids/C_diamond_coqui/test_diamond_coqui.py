...
def not_closed_thc_with_noncollinear_wfn(case:AFQMCInputSet):
    """
    Filters out cases where the Hamiltonian is Closed and THC and the wavefunction is non-collinear,
        since this combination is known to be not implemented.
    """
    if case.hamiltonian.type == HamiltonianClass.THC and case.hamiltonian.spin_symm == SpinSymm.CLOSED:
...
def hamil_type_dictionary():
    ...
    """
    return {
        "ham_chol_closed" :  AFQMCHamiltonian(
        ...
        "ham_thc_closed" :  AFQMCHamiltonian(
            INPUTS_DIR/"ham_thc_1e-6.h5",
            SpinSymm.CLOSED,
            HamiltonianClass.THC,
            runparams={
                'max_num_mpi_ranks':16,# Must have fewer MPI ranks than number of THC columns!
                'total_walkers':3200
            } 
        )
    }
...
def wavefunction_type_dictionary():
    ...
    """
    return {
        "pbe_closed_nomsd" : AFQMCWavefunction(
            INPUTS_DIR/"wfn_mf_pbe_closed.h5",
            SpinSymm.CLOSED,
            WavefunctionClass.NOMSD
        ),
        "pbe_collinear_nomsd": AFQMCWavefunction(
            INPUTS_DIR/"wfn_mf_pbe.h5",
            SpinSymm.COLLINEAR,
            WavefunctionClass.NOMSD
        ),
        "pbe_collinear_nomsd_noncollinear": AFQMCWavefunction(
            INPUTS_DIR/"wfn_mf_pbe_noncollinear.h5",
        ...
        for trial_wfn_name in wavefunction_type_dictionary:
            ...
            for walker in walker_type_list:
                ...
                if not walker_path.exists:
                    ...
                    continue
                # get custom run parameters if they exist - Walker, Wavefunction, Hamiltonian is the order of precedence
