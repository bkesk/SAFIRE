...
def extract_qmc_dm(filename, dm_name='Mixed'):
    r"""Extract AFQMC 1RDM from file.
    ...
    with h5py.File(filename, 'r') as fh5:
        ...
        for G in data.keys():
            if 'denominator' in G:
                ...
                # TODO: use SlaterType enum instead of raw ints
                if walker_type == 2:
...
def get_metadata(filename):
    r"""Extract QMC estimator metadata from h5 file.
...
def extract_rdm_single(filename, indx, dm_name='Mixed'):
    r"""Extract single sample of QMC 1RDM.
...
def get_one_rdm_av(filename, skip, name="Mixed"):
    r"""Helper routine to compute averaged qmc density matrix.
    ...
    ----------
    filename : string
        output file containing density matrix (\*.scalar.out file).
    skip : float
...
def get_qmc_dm_trace(filename, dm_name):
    r"""Get trace of spin up and down 1RDM as function tau. Useful for debugging.
