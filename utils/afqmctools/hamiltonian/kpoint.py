...
def write_hamil_kpoints(comm, scf_data, hamil_file, chol_cut,
    ...
    with FileHandler(comm,hamil_file,"w",phdf) as h5file:
        ...
        if comm.rank == 0 and verbose:
            print(" # Time to reach Cholesky: {:13.8e} s.".format(time.time()-tstart))
            sys.stdout.flush()
        tstart = time.time()
        ...
        if comm.rank == 0 and verbose:
            print(" # Time to perform Cholesky: {:13.8e} s.".format(time.time()-tstart))
            sys.stdout.flush()
        ...
        if not phdf and comm.rank==0:
            ...
            for rk in range(1,comm.size):
                with FileHandler(comm,"rank"+str(rk)+"_"+hamil_file,"r",False) as h5f2:
                    for Q in range(nkpts):
                        if Q > kminus[Q]:
                            continue
                        Ldim = h5f2["/Hamiltonian/KPFactorized/Ldim"+str(Q)][:]
                        nkk = Ldim[0]
                        nij = Ldim[1]
                        kk0 = Ldim[2]
                        ij0 = Ldim[3]
                        ijN = Ldim[4]
                        numv = Ldim[5]
                        LQ2 = h5f2["/Hamiltonian/KPFactorized/L"+str(Q)][:]
...
def write_rhoG_kpoints(comm, scf_data, hdf_file, Gcut,
    ...
    with FileHandler(comm,hdf_file,"w",phdf) as h5file:
        h5file.grp = h5file.create_group("rhoG")
        if h5file.error:
            sys.exit()
        write_basic(comm, cell, kpts, hcore, h5file, X, nmo_pk,
                    qk_to_k2, kminus, verbose=verbose, nelec=nelec)
    ...
    #    solver.run(comm, X, h5file)
        ...
        if not phdf and comm.rank==0:
            ...
            nkpts = len(kpts)
            for rk in range(1,comm.size):
                with FileHandler(comm,"rank"+str(rk)+"_"+hdf_file,"r",False) as h5f2:
                    for Q in range(nkpts):
                        if Q > kminus[Q]:
                            continue
...
class FileHandler:
    """
    A file handler for parallel hdf5 files.
    ...
    def __enter__(self):
        ...
        """
        def _decorate(f):
            setattr(f,"phdf",self._phdf)
            setattr(f,"error",self.error)
            return f

        return _decorate(self.h5f)
...
class KPCholesky(object):
    '''
    Object to perform Cholesky factorization of the 2-body integrals for kpoint
    Hamiltonian.
    ...
    comm : mpi comm
        MPI communicator.
    cell : PySCF pbc.gto.Cell instance
        PySCF cell object.
    kpts : list of kpoints
        List of kpoints.
    maxvecs : int
        *Multiplier* for maximum number of Cholesky vectors. Maximum number of
        Cholesky vectors = maxvecs*nmo_max.
    nmo_pk : list of int
        Number of MOs per kpoint.
    QKToK2 : numpy.ndarray
        Mapping from QK to k2.
        ...
        for Q in range(nkpts):
            ...
            if h5file.phdf or comm.rank==0:
                ...
                for kk in range(part.nkk):
                    T_ = to_complex(cholvecs[kk,:,0:numv].copy())
