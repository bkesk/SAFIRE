...
unless the executables resolve. See ``ExternalTools`` in ``_common`` for the
environment variables; ``make_inputs.py --list`` reports what is missing.
...
from ._common import ASSETS, run_external
...
def _run_quantum_espresso(ctx: BuildContext, kgrid: str) -> Path:
    ...
    ctx.tools.require("pw_x", "pw2coqui_x")
